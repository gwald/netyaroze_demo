...
void CreateStripedTextureTwo (void);
...
void CreateSomeObjects (void)
{
	...
	object = GetNextFreeObject();
	assert(object != NULL);
	MakeLivingObjectOfTimFile(object, &StripedTwo, 0, 80);
	printf("id: %d\n", object->id);
	...
	//PrintClut (StripedTwo.cx, StripedTwo.cy, 8);
}
...
u_short clut2[256];
				...
				// for 8-bit, use CHAR pixels: each one just 8 bits
				// for 4-bit, squash 2 pixels into one char
...
void CreateStripedTextureTwo (void)
{
	...
	for (i = 0; i < 64; i++)
		...
		if (i < 21)
			...
			clut2[i] = ((i - 43) << 10) + ((64 - i) << 5);	
		#endif	
		}

	StripedTwo.pmode = 9;			// 8-bit flag

	setRECT( &pixelRect, 320, 128, 32, 64);
	StripedTwo.px = pixelRect.x;
	StripedTwo.py = pixelRect.y;
	StripedTwo.pw = pixelRect.w;
	StripedTwo.ph = pixelRect.h;

	pixelPointer = (u_long*) pixels3;
	StripedTwo.pixel = (u_long*) pixels3;	
	LoadImage( &pixelRect, pixelPointer);

	setRECT( &clutRect, 320, 256, 256, 1);
	StripedTwo.cx = clutRect.x;
	StripedTwo.cy = clutRect.y;
	StripedTwo.cw = clutRect.w;
	StripedTwo.ch = clutRect.h;

	clutPointer = (u_long*) clut2;
	StripedTwo.clut = (u_long*) clut2;	
	LoadImage( &clutRect, clutPointer);
