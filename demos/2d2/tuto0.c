...
void DealWithControllerPad (void)
{
	...
	if (pad & PADL1)
		{
		RotateAllObjects (ONE, ANTICLOCKWISE);
		return;
		}
	if (pad & PADR1)
		{
		RotateAllObjects (ONE, CLOCKWISE);
		return;
		}
}
...
void CreateSomeObjects (void)
{
	...
	object = GetNextFreeObject();
	assert(object != NULL);
	MakeLivingObjectOfTimFile(object, &MascotsTextureInfo, -100, -20);
	printf("id: %d\n", object->id);

	object = GetNextFreeObject();
	assert(object != NULL);
	MakeLivingObjectOfTimFile(object, &AsciiTextureInfo, -100, 150);
	printf("id: %d\n", object->id);
