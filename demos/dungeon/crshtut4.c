...
void PlayerFiresAShot (void)
{
	...
	if (cubeID == -1)		// no free shots
		...
		{
		TheBullets[cubeID].alive = TRUE;
		TheBullets[cubeID].lifeTime = 0;
		TheBullets[cubeID].allegiance = PLAYER;

			// start spinning
		TheBullets[cubeID].rotationMomentumFlag = TRUE;
		TheBullets[cubeID].twist.vx = rand() % 20;
		TheBullets[cubeID].twist.vy = rand() % 20;
			// send in direction of ship at time of fire
			// NOTE: this goes badly askew when ship not parallel to xz plane
		TheBullets[cubeID].movementMomentumFlag = TRUE;
		TheBullets[cubeID].velocity.vx = PlayersShip.coord.coord.m[2][0] >> PlayerBulletSpeedFactor;
		TheBullets[cubeID].velocity.vy = PlayersShip.coord.coord.m[2][1] >> PlayerBulletSpeedFactor;
		TheBullets[cubeID].velocity.vz = -PlayersShip.coord.coord.m[2][2] >> PlayerBulletSpeedFactor; 
		
					// emitted from nose of ship
		TheBullets[cubeID].position.vx = PlayersShip.position.vx;
		TheBullets[cubeID].position.vy = PlayersShip.position.vy;
		TheBullets[cubeID].position.vz = PlayersShip.position.vz;
		}
}
...
void EnemyShipFiresAShot (ObjectHandler* enemy)
{
	...
	if (cubeID == -1)		// no free shots
		...
		TheBullets[cubeID].alive = TRUE;
		TheBullets[cubeID].lifeTime = 0;
		TheBullets[cubeID].allegiance = ENEMY;
		...
		TheBullets[cubeID].movementMomentumFlag = TRUE;
		TheBullets[cubeID].velocity.vx = enemy->coord.coord.m[2][0] >> EnemyBulletSpeedFactor;
		TheBullets[cubeID].velocity.vy = enemy->coord.coord.m[2][1] >> EnemyBulletSpeedFactor;
		TheBullets[cubeID].velocity.vz = -enemy->coord.coord.m[2][2] >> EnemyBulletSpeedFactor; 
		
					// emitted from nose of ship
		TheBullets[cubeID].position.vx = enemy->position.vx;
		TheBullets[cubeID].position.vy = enemy->position.vy;
		TheBullets[cubeID].position.vz = enemy->position.vz;
		}
}
