...
{	
	...
	antiClockTgt.vz = -radius.vx;
		...
		// add ViewpointZDistance; change sign of tgts
...
{ 
	...
	assert(gridX >= 0 && gridX < WLDX_MAX);
	assert(gridY >= 0 && gridY < WLDY_MAX);
...
void InitialiseView( void )
{
	...
	view.vrx = PlayersShip.position.vx;
	view.vry = 0;
	view.vrz = PlayersShip.position.vz;

	view.vpx = view.vrx;
	view.vpy = view.vry;
	view.vpz = view.vrz + ViewDistance;
