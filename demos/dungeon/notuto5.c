 ...
 ***********************************************************/
 ...
 // changes from tuto4:
 // new way of calc_model_data:
 // put wall objects inside actual wall squares
 // for better collision-with-wall detection

// future extensions: make dungeon 3D, hence make game dynamics
// fully 3D; 
// improve smoothness (comparison with test),
// smoother views and multiple view modes

// demo features: backmirror, ship angle tracking (small rectangle
// in corner of screening showing ship and world axes),
// flash views, playback film, etc

// improve strategies of enemy ships 
...
int WillCollideWithWalls (int x, int y, int dx, int dy);
...
int NewCalculateModelData (int n, int x, int y, int fact, int attribute);
...
{	
	...
	for (x = sx; x <= ex; x++) 
		...
		for (y = sy; y <= ey; y++) 
			...
			if( abs(px-x)<3 && abs(py-y)<3 ) 
				{
				numberOfModels = calc_model_data( numberOfModels, x, y, fact, GsDIV2);
				//numberOfModels = NewCalculateModelData( numberOfModels, x, y, fact, GsDIV1);
				}
			#endif
			if( abs(px-x)<4 && abs(py-y)<4 ) 
				...
				//numberOfModels = NewCalculateModelData( numberOfModels, x, y, fact, 0);
				}
...
int NewCalculateModelData (int n, int x, int y, int fact, int attribute)
{
		// firstly consider edges of world map
		if (x == 0)
			{
			GsInitCoordinate2( &World[x][y], &DWorld[n] );
			DWorld[n].coord.t[0] = 0;
			DWorld[n].coord.t[1] = 0;
			DWorld[n].coord.t[2] = 0;
			GsLinkObject4((long)dop4, &Models[n],0);
			Models[n].coord2 = &DWorld[n];
			Models[n].attribute = attribute;
			n++;
			}
		else if (x == WLDX_MAX-1)
			{
			GsInitCoordinate2( &World[x][y], &DWorld[n] );
			DWorld[n].coord.t[0] = 0;
			DWorld[n].coord.t[1] = 0;
		...
		// always put in ceiling and floor
		// (surely can avoid this if a wall square??)
	GsInitCoordinate2(&World[x][y], &DWorld[n] );				// ceiling
	DWorld[n].coord.t[0] = 0;
	DWorld[n].coord.t[1] = - ( fact );
	DWorld[n].coord.t[2] = 0;
	GsLinkObject4((long)dop5, &Models[n],0);
	...
	GsInitCoordinate2(&World[x][y], &DWorld[n] );				// floor
	DWorld[n].coord.t[0] = 0;
	DWorld[n].coord.t[1] = fact;
	DWorld[n].coord.t[2] = 0;
	GsLinkObject4((long)dop6, &Models[n],0);
	...
	n++;
		...
		// if square not a wall: do nowt
	if ( (worldmaps[x][y] & WALL) == 0) 	   
		{	
		return(n);
		}

		// else square is a wall; put wall object at each side where adjacent square
		// is not a wall

	if ( (worldmaps[x-1][y]&WALL) == 0) 
		{	  // empty square to left
		GsInitCoordinate2( &World[x][y], &DWorld[n] );
		DWorld[n].coord.t[0] = 0;
		DWorld[n].coord.t[1] = 0;
		DWorld[n].coord.t[2] = 0;
	...
	if ( (worldmaps[x][y-1]&WALL) == 0)	 // empty square below
		{
		GsInitCoordinate2( &World[x][y], &DWorld[n] );
		DWorld[n].coord.t[0] = 0;
		DWorld[n].coord.t[1] = 0;
...
{ 
	...
	if (WillCollideWithWalls(position->vx, position->vz, realMovement.vx, realMovement.vz))
		{
		velocity->vx = 0;
		velocity->vy = 0;
		velocity->vz = 0;
...
{
	int gridX, gridY;
	static int objectSize = FACT/4;			// arbitrary dimension: blaggery

	gridX = (x + dx + objectSize) / FACT;
	gridY = (y + dy + objectSize) / FACT;
	if (gridX < 0 || gridY < 0 || gridX > WLDX_MAX-1 || gridY > WLDY_MAX-1)
		return 1;
	if (worldmaps[gridX][gridY] & WALL)
		return 1;
