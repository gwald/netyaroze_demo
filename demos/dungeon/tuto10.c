...
// ONLY change from tuto9:
// includes 6 tracer cubes which mark the ship's relative axes
// ie test the 6 functions at bottom of the file
...
#define MAX_TRACERS 6
ObjectHandler TheTracers[MAX_TRACERS];
...
void InitialiseTracerElement (ObjectHandler* tracer, int whichOne);

void HandleAllObjects (void);

void UpdateTracerPosition (ObjectHandler* object);
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		...
		TheShips[i].currentHealth = 1; 
			...
			// currently disabled: strategies need updating to 3D world
	...
	for (i = 0; i < MAX_TRACERS; i++)
		{
		InitSingleObject(&TheTracers[i]);

		BringObjectToLife(&TheTracers[i], TRACER, 
			CUBE_MODEL_ADDRESS, 0, NONE);

		InitialiseTracerElement(&TheTracers[i], i);

		RegisterObjectIntoObjectArray(&TheTracers[i]);
		}
	printf("5\n");
...
void InitialiseTracerElement (ObjectHandler* tracer, int whichOne)
{
		// all cases same: maybe make axes proper long cuboids
		// rather than cube end-markers
	switch(whichOne)
		{
		case 0: case 1:
			SetObjectScaling(tracer, ONE>>5, ONE>>5, ONE>>5);
			break;
		case 2: case 3:
			SetObjectScaling(tracer, ONE>>5, ONE>>5, ONE>>5);
			break;
	...
	tracer->position.vx = PlayersShip.position.vx;
	tracer->position.vy = PlayersShip.position.vy;
	tracer->position.vz = PlayersShip.position.vz;

	tracer->subType = whichOne;
}
			...
			if (ObjectArray[i]->alive == TRUE)
				...
				switch(object->type)
					...
					case ENEMY:
						...
						break;
					case TRACER:
						object->position.vx = PlayersShip.position.vx;
						object->position.vy = PlayersShip.position.vy;
						object->position.vz = PlayersShip.position.vz;
							
						UpdateTracerPosition(object);
...
void UpdateTracerPosition (ObjectHandler* object)
{
	...
	switch(object->subType)
		{
		case 0:
			GetPlusXAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		case 1:
			GetMinusXAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		case 2:
			GetPlusYAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		case 3:
			GetMinusYAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		case 4:
			GetPlusZAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		case 5:
			GetMinusZAxisFromMatrix( &PlayersShip.matrix, &displacement);
			break;
		default:
			assert(FALSE);
		}	

	object->position.vx += displacement.vx;
	object->position.vy += displacement.vy;
	object->position.vz += displacement.vz;

	if (frameNumber % 15 == 0)
		{
		printf("tracer subtype %d\n");
		printf("position offset: %d %d %d\n", 
...
int NotSafe (CUBOID* cuboid)
{
	...
	y1 = (cuboid->y) / FACT;
	z1 = (cuboid->y) / FACT;
	x2 = (cuboid->x + cuboid->w) / FACT;
	y2 = (cuboid->y + cuboid->h) / FACT;
	z2 = (cuboid->y + cuboid->d) / FACT;

	if (x1 < 0 || x1 >= WORLD_MAX_X)
		return TRUE;
	if (y1 < 0 || y1 >= WORLD_MAX_Y)
		return TRUE;
	if (z1 < 0 || z1 >= WORLD_MAX_Z)
		return TRUE;
	if (x2 < 0 || x2 >= WORLD_MAX_X)
		return TRUE;
	if (y2 < 0 || y2 >= WORLD_MAX_Y)
		return TRUE;
	if (z2 < 0 || z2 >= WORLD_MAX_Z)
		return TRUE;
...
void PositionSomewhereNotInAWall (ObjectHandler* object)
{
	...
	xOffset = FACT/2;//FACT/4 + (rand() % (FACT/2));
	yOffset = FACT/2;//FACT/4 + (rand() % (FACT/2));
	zOffset = FACT/2;//FACT/4 + (rand() % (FACT/2));
