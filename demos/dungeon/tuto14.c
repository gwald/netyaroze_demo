...
int NotSafe (CUBOID* cuboid)
{
	...
	x1 = (cuboid->x - (cuboid->w/2)) /FACT;	  
	y1 = (cuboid->y - (cuboid->h/2)) /FACT;
	z1 = (cuboid->z - (cuboid->d/2)) /FACT;
	x2 = (cuboid->x + (cuboid->w/2)) /FACT;
	y2 = (cuboid->y + (cuboid->h/2)) /FACT;
	z2 = (cuboid->z + (cuboid->d/2)) /FACT;
...
void PositionSomewhereNotInAWall (ObjectHandler* object)
{
...
#if 0			// OLD: for given 3 by 3 by 3 block walls
		if (worldmaps[gridX][gridY][gridZ] != WALL
	...
	xOffset = FACT/2;//+ (rand() % (FACT/2));
	yOffset = FACT/2;//+ (rand() % (FACT/2));
	zOffset = FACT/2;// + (rand() % (FACT/2));
