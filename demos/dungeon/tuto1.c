...
void DealWithControllerPad (void)
{
	...
	switch(MainMode)
		...
		case MOVE_SHIP:
			...
			if (pad & PADR2)
				{
				view.vrx = 0; view.vry = 0; view.vrz = rDist;
				view.vpx = 0; view.vpy = 0; view.vpz = pDist;
				view.rz = 0;
...
{
	TheLights[0].r = TheLights[0].g = TheLights[0].b = 0x70;
	TheLights[0].vx = PlayersShip.coord.coord.m[2][0];
	TheLights[0].vy = 4096;			 // from above
	TheLights[0].vz = PlayersShip.coord.coord.m[2][2];
	GsSetFlatLight(0,&TheLights[0]);

	TheLights[1].r = TheLights[1].g = TheLights[1].b = 0x70;
	TheLights[1].vx = PlayersShip.coord.coord.m[2][0];
	TheLights[1].vy = -4096;			 // from below
	TheLights[1].vz = PlayersShip.coord.coord.m[2][2];
	GsSetFlatLight(1,&TheLights[1]);

	TheLights[2].r = TheLights[2].g = TheLights[2].b = 0x70;
	TheLights[2].vx = -PlayersShip.coord.coord.m[2][0];
	TheLights[2].vy = 4096;			 // from above
	TheLights[2].vz = -PlayersShip.coord.coord.m[2][2];
...
{	
	...
	radius.vx = PlayersShip.coord.coord.m[2][0];
	radius.vy = PlayersShip.coord.coord.m[2][1];
	radius.vz = PlayersShip.coord.coord.m[2][2];
...
#if 0
	p1.vx = PlayersShip.position.vx + ((radius.vx + antiClockTgt.vx) >> 3);
	p1.vy = PlayersShip.position.vy;
	p1.vz = PlayersShip.position.vz + ((radius.vz + antiClockTgt.vz) >> 3);
	p2.vx = PlayersShip.position.vx + ((radius.vx + clockTgt.vx) >> 3);
	p2.vy = PlayersShip.position.vy;
	p2.vz = PlayersShip.position.vz + ((radius.vz + clockTgt.vz) >> 3);
#endif
	p1.vx = PlayersShip.position.vx + ((-2 * radius.vx + antiClockTgt.vx) >> 3);
	p1.vy = PlayersShip.position.vy;
	p1.vz = PlayersShip.position.vz + ((-2 * radius.vz + antiClockTgt.vz) >> 3);
	p2.vx = PlayersShip.position.vx + ((-2 * radius.vx + clockTgt.vx) >> 3);
	p2.vy = PlayersShip.position.vy;
	p2.vz = PlayersShip.position.vz + ((-2 * radius.vz + clockTgt.vz) >> 3);
#if 1
	p3.vx = PlayersShip.position.vx + ((-radius.vx + antiClockTgt.vz) >> 3);
	p3.vy = PlayersShip.position.vy;
	p3.vz = PlayersShip.position.vz + ((-radius.vx + antiClockTgt.vx) >> 3);
	p4.vx = PlayersShip.position.vx + ((-radius.vx + clockTgt.vx) >> 3);
	p4.vy = PlayersShip.position.vy;
	p4.vz = PlayersShip.position.vz + ((-radius.vz + clockTgt.vz) >> 3);
#endif
			...
			if (ObjectArray[i]->alive == TRUE)
				...
				switch(object->type)
					...
					case PLAYER:
						...
						object->velocity.vx += object->twist.vx;
						object->velocity.vy += object->twist.vy;
						object->velocity.vz += object->twist.vz;
...
#if 0
	view.vrx = 0; view.vry = 0; view.vrz = -300;
	view.vpx = 0; view.vpy = 0; view.vpz = - 50;
	view.rz = 0;
	view.super = &PlayersShip.coord;
#endif
