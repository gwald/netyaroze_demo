...
ObjectHandler SpecialObject;		// 3d sprite object
...
void HandleASpecial (ObjectHandler* object);
...
u_short SortSpriteObjectPosition (ObjectHandler* object, GsOT *ot);
				...
				if (ObjectArray[i]->alive == TRUE)
					...
					if (object->displayFlag == TMD)
						...
						GsSortObject4( &(object->handler), 
								&Wot[bufferIndex], 
									MAX_OT_LENGTH - Wot[bufferIndex].length, getScratchAddr(0));
						}
					else if (object->displayFlag == SPRITE)	  // speed-up here: use a fast flag
						{	  // some can use GsSortFastSprite
						zValue = SortSpriteObjectPosition(object, &Wot[bufferIndex]);
...
void InitialiseObjects (void)
{
	...
	InitSingleObject(&SpecialObject);

	BringObjectToLife(&SpecialObject, SPECIAL, 
		CUBE_MODEL_ADDRESS, 0, NONE);

	SpecialObject.displayFlag = SPRITE;
	SpecialObject.imageInfo = &FireTextureInfo;
	SpecialObject.scaleX = ONE * 4;
	SpecialObject.scaleY = ONE * 4;

	RegisterObjectIntoObjectArray(&SpecialObject);
	...
	//SpecialObject.sprite.w = 2;
	//SpecialObject.sprite.h = 2;

	setVECTOR( &SpecialObject.position, 600, 600, -600);
}
...
void DealWithControllerPad (void)
{
	...
	if (pad & PADselect)
		...
		// L2 modifier: move 3dsprite object
	if (pad & PADL2)
		{
		if (pad & PADRleft)
			{
			SpecialObject.position.vx -= 25;
			}
		if (pad & PADRright)
			{
			SpecialObject.position.vx += 25;
...
u_short SortSpriteObjectPosition (ObjectHandler* object, GsOT *ot)
{
	...
	if (screen.vz < ProjectionDistance/2)
		...
		TURN_NTH_BIT_OFF(object->sprite.attribute, 32, 31)	// display on: ensure 31st bit is OFF

	adjust = 16 - ot->length;
	zValue = (u_short) (screen.vz >> adjust);	 								
		...
		if (polygon != NULL)
			...
			if (polygon->alive == TRUE)
				...
				GsSortObject4( &(polygon->handler), 
						ot, MAX_OT_LENGTH - ot->length, getScratchAddr(0));
				}
			}
		}
}
