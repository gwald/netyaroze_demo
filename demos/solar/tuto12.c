...
#define STANDARD_VIEW 0
#define CIRCLING_VIEW 1
#define FLYBY_VIEW 2
#define DIRECTORS_VIEW 3

			
	// the axes' ends: used by the standard views
#define PLUS_X 0
#define MINUS_X 1
#define PLUS_Y 2
#define MINUS_Y 3
...
u_long numberFramesSinceViewpointChange;
	...
	// special views: standard, circling, fly-by, director's cut
int SpecialViewFlag;
int SpecialViewType;	

	// standard: six views	
int WhichStandardView;
int StandardViewDistance;
...
int DirectorSequences[2][3] = { {STANDARD_VIEW, CIRCLING_VIEW, FLYBY_VIEW},
								{STANDARD_VIEW, FLYBY_VIEW, CIRCLING_VIEW} };
...
void SetUpStandardView (int whichStandardView, int standardViewDistance);
		...
		case OBJECT_VIEWS:
			switch(SpecialViewType)
				...
				case NONE:
					...
					break;
				case STANDARD_VIEW:
					FntPrint("View: Standard\n");

					switch(WhichStandardView)
						{
						case PLUS_X:
							FntPrint("From right\n");
			   				break;
					...
					FntPrint("view distance: %d\n", StandardViewDistance);
					break;
				case CIRCLING_VIEW:	
					FntPrint("View: Circling\n");
					...
					break;
				case FLYBY_VIEW:	
					FntPrint("View: Fly-by\n");

					switch(TypeOfFlybyView)
						{
						case CIRCULAR:
							FntPrint("object is orbiting\n");
							break;
						case LINEAR:
							FntPrint("object moves straight\n");
							break;
						default:
							assert(FALSE);
						}
					...
					FntPrint("frame in cycle: %d\n", 
								NumberFlybyFramesSoFar);
					break;
				case DIRECTORS_VIEW:	
					FntPrint("View: Director's cut\n");
					break;
				default:
					assert(FALSE);
				}
	...
	if (PadStatus & PADstart)
		...
		if (framesSinceLastSwitch > 10)		
			...
			if (SelectionMode == SELECT_OBJECT)
				{
					// check SelectedObject id, un-highlight the right one
				assert(SelectedObject >= 0);
				assert(SelectedObject < MAX_OBJECTS);
			...
			if (SelectionMode == SELECT_OBJECT)
				{
					// check SelectedObject id, highlight the right one
				assert(SelectedObject >= 0);
				assert(SelectedObject < MAX_OBJECTS);
   		...
   		// L2 and Rdown held down together: quit
	if (PadStatus & PADL2 && PadStatus & PADRdown) 
		{
		quitCounter++;
		if (quitCounter > 15)			   
			return 0;
		...
		// L2 and R2 and No start and No select:
		// reset all to initial state
	if (PadStatus & PADL2 && PadStatus & PADR2
		&& ((PadStatus & PADselect) == 0) && ((PadStatus & PADstart) == 0) ) 
		{
		ResetAll();
		}
...
void HandleLightControls (void)
{
	...
	if (PadStatus & PADR2)
		...
		// select and start: select next light
	if (PadStatus & PADstart && PadStatus & PADselect)
		{
		if (framesSinceLastLightChange > 15)	   
			{
			if (SelectedLight == 2)	
				SelectedLight = 0;
			else
...
void HandleFoggingControls (void)
{
	...
	if (PadStatus & PADRright)
		...
		// start and select: make for colour fully white or black
	if (PadStatus & PADstart && PadStatus & PADselect)
		{
...
void HandleObjectSelection (void)
{
		// select and R1/R2: select next submode
	...
	if (PadStatus & PADR2 && PadStatus & PADselect)
		{
		if (framesSinceLastSelection > 12)
			{
		...
		// R1 alone: select next object up
	if (PadStatus & PADR1 && ((PadStatus & PADselect) == 0) )
		{
		...
		// R2 alone: select next object down
	if (PadStatus & PADR2 && ((PadStatus & PADselect) == 0) )
		{
		if (framesSinceLastSelection > 5)
			{
			UnHighlightObject( &ObjectArray[SelectedObject]);
	...
	switch(ObjectSelectionMode)
		...
		case CHANGE_DISPLAY:
			...
			break;
		case OBJECT_VIEWS:
				// options: standard view (above, left, front, etc)
				// circling: which plane, radius, speed, etc
				// flyby; director's cut

				// L1 alone: set to standard view
			if (PadStatus & PADL1 && ((PadStatus & PADselect) == 0) )
				{
				if (framesSinceLastSelection > 10)
					{
					TheMainView.super = WORLD;
					SetUpStandardView (WhichStandardView, 
											StandardViewDistance);
					framesSinceLastSelection = 0;
					}
				}

				// L2 alone: set to circling view
			if (PadStatus & PADL2 && ((PadStatus & PADselect) == 0) )
				{
				if (framesSinceLastSelection > 10)
					{
					TheMainView.super = WORLD;
				...
				// L1 and select: set to fly-by view
			if (PadStatus & PADL1 && PadStatus & PADselect)
				{
				if (framesSinceLastSelection > 10)
					{
					TheMainView.super = WORLD;
				...
				// L2 and select: set to fly-by view
			if (PadStatus & PADL2 && PadStatus & PADselect)
				{
				if (framesSinceLastSelection > 10)
					{
					TheMainView.super = WORLD;
				...
				// start and select: set a normal i.e. movable view
			if (PadStatus & PADstart && PadStatus & PADselect)
				{
				if (framesSinceLastSelection > 10)
					{
...
void HandleStandardView (void)
{
		// Lleft/Lright: change which standard view
	if (PadStatus & PADLleft)
		{
		if (framesSinceLastSelection > 10)
			{
			if (WhichStandardView == PLUS_X)
				WhichStandardView = MINUS_Z;
			else
				WhichStandardView--;
			framesSinceLastSelection = 0;
			}
	...
	if (PadStatus & PADLright)
		...
		// Lup/Ldown: change view distance
	if (PadStatus & PADLup)
		{
		StandardViewDistance += 50;
		}
	if (PadStatus & PADLdown)
	   	{
		StandardViewDistance -= 50;
		if (StandardViewDistance < 0)
		StandardViewDistance = 0;
		}
}
...
void HandleCirclingView (void)
{
		// Lleft: change which plane of viewing orbit
	if (PadStatus & PADLleft)
		{
		if (framesSinceLastSelection > 10)
			{
		...
		// Lup: toggle which way round the view orbits
	if (PadStatus & PADLup)
		{
		if (framesSinceLastSelection > 10)
			{
		...
		// Rup/Rdown: change circling view radius
	if (PadStatus & PADRup)
		{
		CircleViewRadius += 50;
		}
	if (PadStatus & PADRdown)
		{
		CircleViewRadius -= 50;
		if (CircleViewRadius < 500)
			CircleViewRadius = 500;
		}
						
		// Rleft/Rright: change circling orbital speed
	if (PadStatus & PADRleft)
		{
		CircleViewAngleIncrement += 2;
		}
	if (PadStatus & PADRright)
		{
		CircleViewAngleIncrement -= 2;
		if (CircleViewAngleIncrement < 2)
			CircleViewAngleIncrement = 2;
		} 
}
...
void HandleFlybyView (void)
{
		// Rup/Rdown: change path distance
	if (PadStatus & PADRup)
		{
		PathDistance += 50;
		}
	if (PadStatus & PADRdown)
		...
		// Rleft/Rright: change path angle
	if (PadStatus & PADRleft)
		{
		PathAngle += 16;
		}
	if (PadStatus & PADRright)
		...
		// Lup/Ldown: change flyby cycle time
	if (PadStatus & PADLup)
		{
		FlybyCycleTime++;
		}
...
void InitialiseView (void)
{
	...
	SpecialViewType = NONE;
		...
		// default: look from above
	WhichStandardView = MINUS_Y;
		// long way away to see surrounding bodies
	StandardViewDistance = 12000;
...
void SetUpStandardView (int whichStandardView, int standardViewDistance)
{
	...
	SpecialViewType = STANDARD_VIEW;

	assert(whichStandardView >= PLUS_X
			&& whichStandardView <= MINUS_Z);
	
	WhichStandardView = whichStandardView;
	StandardViewDistance = standardViewDistance;
}	
...
void DealWithStandardView (void)
{
	...
	switch(WhichStandardView)
		{
		case PLUS_X:
			displacement.vx = StandardViewDistance;
			break;
		case MINUS_X:
			displacement.vx = -StandardViewDistance;
			break;
		case PLUS_Y:
			displacement.vy = StandardViewDistance;
			break;
		case MINUS_Y:
			displacement.vy = -StandardViewDistance;
			break;
		case PLUS_Z:
			displacement.vz = StandardViewDistance;
			break;
		case MINUS_Z:
			displacement.vz = -StandardViewDistance;
			break;
