...
void HandleFoggingControls (void)
{
	...
	if (PadStatus & PADRright)
		...
		// start: make for colour fully white or black
	if (PadStatus & PADstart)
		{
...
void HandleViewControls (void)
{
	...
	if (PadStatus & PADselect)	   // how fast viewpoint changes
		...
		// start: choose next view mode
	if (PadStatus & PADstart)
		{
		if (numberFramesSinceViewModeChange > 10)
			{
	...
	switch(ViewMode)
		{
		case GENERAL_DISTORTION:
				// Rup/Rdown: change projection distance
			if (PadStatus & PADRup)
				{
			...
			if (PadStatus & PADRdown)
				...
				// Ldown/Lup: change the 'rz' distortion
			if (PadStatus & PADLup)
				{
			...
			break;
		case MOVE_BOTH_ABSOLUTELY:
				// six left-pad directional controls:
				// move the reference point absolutely
			if (PadStatus & PADLleft)
				{
				TheMainView.vrx += viewChange;
				ViewChanged = TRUE;
				}
			if (PadStatus & PADLright)
				{
			...
			if (PadStatus & PADL2)
				...
				// six right-pad directional controls:
				// move the viewing point absolutely
			if (PadStatus & PADRleft)
				{
		...
		case CHANGE_SUPER:
			...
			if (PadStatus & PADRup)
				...
				// Rdown: view from next object down
			if (PadStatus & PADRdown)
				{
				if (numberFramesSinceViewpointChange > 20)
					{
				...
				// R1: toggle WORLD/player's ship/objects
				// circular list: (WORLD, player's ship, objects)
			if (PadStatus & PADR1)
				{
				if (numberFramesSinceViewpointChange > 20)
					{
				...
				// Ldown: force return to WORLD view
			if (PadStatus & PADLdown)
				{
				if (numberFramesSinceViewpointChange > 20)
					{
	...
	switch(ObjectSelectionMode)
		...
		case OBJECT_VIEWS:
			...
			if (PadStatus & PADL2)
				...
				// Rup: force return to default i.e. WORLD view
			if (PadStatus & PADRup)
				{
				if (framesSinceLastSelection > 20)
					{
				...
				// L1 alone: set to external view
			if (PadStatus & PADL1 && ((PadStatus & PADselect) == 0) )
				{
				if (framesSinceLastSelection > 10)
					{
					TheMainView.super = WORLD;
					SetUpExternalView (WhichExternalView, 
											ExternalViewDistance);
					framesSinceLastSelection = 0;
					}
				}
...
void HandleExternalView (void)
{
		// Lleft/Lright: change which external view
	if (PadStatus & PADLleft)
		{
		if (framesSinceLastSelection > 10)
			{
	...
	if (PadStatus & PADLup)
		{
		ExternalViewDistance += 50;
		}
	if (PadStatus & PADLdown)
	   	{
		ExternalViewDistance -= 50;
		if (ExternalViewDistance < 0)
		ExternalViewDistance = 0;
		}
}
...
void HandleFlybyView (void)
{
	...
	if (PadStatus & PADLdown)
		...
		FlybyCycleTime--;
 		 ...
 		 // CONTROL CONFLICTS
		 // need to sort selection:
		 // highlighted font, up and down, left/right flip value
 ...
 void HandleLocalView (void)
 {
	...
	if (PadStatus & PADselect)	   
		...
		// left directional controls: alter local angle
  	if (PadStatus & PADLleft)
		{
	...
	if (PadStatus & PADL2)
		...
		// Rup/Rdown: change viewing point distance
	if (PadStatus & PADRup)
		{
	...
	if (PadStatus & PADRdown)
		...
		// Rleft/Rright: change the reference point distance
	if (PadStatus & PADRleft)
		{
		ReferencePointDistance += 10 * speedMultiplier;
		ViewChanged = TRUE;
