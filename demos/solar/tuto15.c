...
char SelectionString[12] = "~cf0f";		   // coloured text strings
...
int SelectedOption;			// the highlighted one
	...
	// sub modes of object manipulation
...
u_long frameNumber = 0;	
	...
	// time gaps between changing variables
...
************************************************************************/
	...
	// NOTE: slow, but very easy to optimise
...
int DealWithController (void)
{
	...
	if (PadStatus & PADselect && PadStatus & PADstart) 
		...
		// start & Rright: step frame by frame
	if (PadStatus & PADstart && PadStatus & PADRright)
		{
...
void InitialiseModels (void)
{
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		if (ObjectArray[i].type >= SUN && ObjectArray[i].type <= MOON)
			{
		...
		else if (ObjectArray[i].type == SHIP)
			{
			LinkObjectHandlerToTmdObject( &(ObjectArray[i].handler), 
					0, SHIP_MODEL_ADDRESS);
			}
		else
			{			// link to first sphere
			LinkObjectHandlerToTmdObject( &(ObjectArray[i].handler), 
					0, SPHERE_MODEL_ADDRESS);
			}
...
void DealWithDirectorsView (void)	 // LARGELY UNFINISHED
{
	int currentSubMode;
...
void SetUpSolarSystem (void)
{
	...
	ObjectArray[ShipID].scaleZ = 4;
		...
		// starting off
	ObjectArray[ShipID].position.vx = 0;
	ObjectArray[ShipID].position.vy = -HALF_RECTANGLE;
	ObjectArray[ShipID].position.vz = 0;

	ObjectArray[ShipID].velocity.vx = 0;
	ObjectArray[ShipID].velocity.vy = 0;
	ObjectArray[ShipID].velocity.vz = 100;
