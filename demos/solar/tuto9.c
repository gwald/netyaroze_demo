...
#define SHUTTLE_MODEL_ADDRESS 			(0x800d0000)
...
void WriteObjectSelectionInfo (void)
{
	...
	switch(ObjectSelectionMode)
		...
		case CHANGE_DISPLAY:
			...
			// NOTE: at present, highlighting the selected object
			// involved turning the bit 6 on. Hence, the means to
			// toggle bit 6 below don't work, since the control
			// affects the selected object. The others do work however.
...
void HandleAllObjects (void)
{
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		// deal with movement and rotation
	UpdateCoordinateSystem2( &PlayersShip.rotate,
							&PlayersShip.twist,
...
void InitialiseModels (void)
{
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		if (ObjectArray[i].type != NONE)
			whichModel = 2 - ObjectArray[i].type;
		else
			whichModel = 0;	
