...
typedef struct
{
	...
	int theta1, theta2;	// angles describing plane of orbit
	int increment1, increment2;
	int whichWay1, whichWay2;		// CLOCKWISE, ANTICLOCKWISE
} ObjectHandler;
...
void PutObjectIntoOrbit (ObjectHandler* object, int centralBodyID,
				int radius, int theta1, int theta2,
						int increment1, int increment2,
							int whichWay1, int whichWay2);
...
void DealWithOrbitalMovement (void)
{
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		// NEW: proper 3-D orbits	DOES NOT WORK PROPERLY
	ObjectHandler* object;
	long x, y, z;
	int i;
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		if (ObjectArray[i].alive == TRUE
			...
			object = &ObjectArray[i];
				...
				// update angles
			switch (object->whichWay1)
				{
				case CLOCKWISE:
					object->theta1 += object->increment1;
					break;
				case ANTICLOCKWISE:
					object->theta1 -= object->increment1;
					break;
			...
			switch (object->whichWay2)
				{
				case CLOCKWISE:
					object->theta2 += object->increment2;
					break;
				case ANTICLOCKWISE:
					object->theta2 -= object->increment2;
					break;
			...
			x = object->radius * rcos(object->theta1) 
						* (rcos(object->theta2));
			y = object->radius * rsin(object->theta2); 
			z = object->radius * rsin(object->theta1) 
						* rcos(object->theta2);

			object->position.vx = x >> 24;
			object->position.vy = y >> 12;
			object->position.vz = z >> 24;
			}
		}
}
...
void InitSingleObject (ObjectHandler* object)
{
	...
	object->increment1 = 0;
	object->increment2 = 0;
	object->whichWay1 = NONE;
	object->whichWay2 = NONE;
}
							...
							int whichWay1, int whichWay2)
{
	...
	object->increment1 = increment1;
	object->increment2 = increment2;
	object->whichWay1 = whichWay1;
	object->whichWay2 = whichWay2;
}	
...
void SortObjectSize (ObjectHandler* object)
{
	...
	scalingVector.vx = ONE * object->scaleX;
	scalingVector.vy = ONE * object->scaleY;
	scalingVector.vz = ONE * object->scaleZ;
