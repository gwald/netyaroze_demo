...
void InitialiseTextStrings (void)
{
	...
	for (i = 0; i < MAX_TEXT_STRINGS; i++)
		...
		AllStrings[i].colour = NORMAL_COLOUR;
		AllStrings[i].data[0] = '\n';
		}
		...
		if (AllStrings[i].length != 0)
			...
			for (j = 0; j < thisOne->length; j++)
				...
				if (sprite != NULL)
					...
					sprite->r = (thisOne->colour & 31) * 4;
					sprite->g = ((thisOne->colour >> 5) & 31) * 4;
					sprite->b = ((thisOne->colour >> 10) & 31) * 4;
