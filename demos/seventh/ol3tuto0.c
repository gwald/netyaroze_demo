...
void InitialiseTheTunnel (void)
{
...
#if 0		// no real need for this overzealous check
	if (ONE % NUMBER_SHAPES_PER_SECTION != 0)
		assert(FALSE);
#endif
	...
	for (i = 0; i < MAX_TUNNEL_SECTIONS; i++)
		...
		for (j = 0; j < NUMBER_SHAPES_PER_SECTION; j++)
			{
			setVECTOR( &TheTunnel[i].pointsOnCircle[j], 0, 0, 0);
			}
		}
	...
	for (i = 0; i < NumberTunnelSections; i++)
		...
		//dumpVECTOR( &TunnelDirectionVectors[i]);

			// now calculate the rotations from each section to the next;
			// treat tunnel as the path followed by a ship that can only
			// affect its movement by rotating around x and y axes
		}
			...
			SVECTOR* rotation, GsCOORDINATE2* output)
{
	...
	setVECTOR( &realRotation, rotation->vx, rotation->vy, 0);
		...
		// find matrix that expresses our rotation
