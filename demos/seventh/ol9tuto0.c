...
#define MAX_TEST_SPRITES 32
ObjectHandler TheTestSprites[MAX_TEST_SPRITES];
...
void HandleASprite (ObjectHandler* object);
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_TEST_SPRITES; i++)
		{
		InitSingleObject(&TheTestSprites[i]);

		BringObjectToLife(&TheTestSprites[i], TEST_SPRITE, 
			SQUARE_MODEL_ADDRESS, 0, NONE);

		TheTestSprites[i].displayFlag = SPRITE;
		TheTestSprites[i].imageInfo = &WaveTextureInfo;
		TheTestSprites[i].sprite.scalex = ONE/8;
		TheTestSprites[i].sprite.scalex = ONE/8;
	
		RegisterObjectIntoObjectArray(&TheTestSprites[i]);
		}
	...
	for (i = 0; i < MAX_TEST_SPRITES; i++)
		{
		TheTestSprites[i].alive = FALSE;		// start off dead
		TheTestSprites[i].sprite.mx = TheTestSprites[i].sprite.w / 2;
		TheTestSprites[i].sprite.my = TheTestSprites[i].sprite.h / 2;
		}

	TheCubes[0].alive = TRUE;
	TheTestSprites[0].alive = TRUE;
}
...
void FindCompoundRotationAngles (int dx, int dy, int dz, int* thetaX, int* thetaY)
{
	...
	int newDX, newDY;
	GsCOORDINATE2 coord, temp;
	SVECTOR rotationVector;
	VECTOR oldPosition, newPosition;
	MATRIX tempMatrix;
	...
	setVECTOR( &oldPosition, dx, dy, dz);
	...
	if (dx > 0)
		...
		for (;;)
			{
			angleY++;				 // NOTE: increment by more than one, or Very slow ....
			setVECTOR( &rotationVector, 0, angleY, 0);
			RotMatrix( &rotationVector, &tempMatrix);
			coord.coord = tempMatrix;
			#if 0
			...
			ApplyMatrixLV( &coord.coord, &oldPosition, &newPosition);
			newDX = newPosition.vx;
			assert(newDX < oldPosition.vx);		// must be getting closer
...
void DealWithControllerPad (void)
{
	...
	if (pad & PADL1 && pad & PADL2)
		...
		// L1 and L2: move the cube, watch sprite follow it
	if (pad & PADL1 && pad & PADL2)
		{
		if (pad & PADRup)
			{
			TheCubes[0].position.vy += 10 * controlSpeed;
			return;
			}
		if (pad & PADRdown)
			{
		...
		if (pad & PADR2)
			{
			TheCubes[0].position.vz += 10 * controlSpeed;
			return;
			}
		if (pad & PADR1)
			{
		...
		if (pad & PADRleft)
			{
			TheCubes[0].position.vx -= 10 * controlSpeed;
			return;
			}
		if (pad & PADRright)
			{
...
void HandleASprite (ObjectHandler* object)
{
	assert(object->type == TEST_SPRITE);

	if (object == &TheTestSprites[0])		 // make it follow cube
		{
		setVECTOR( &object->position, TheCubes[0].position.vx,
			TheCubes[0].position.vy, TheCubes[0].position.vz);
		object->position.vz = 1;		// very close to screen 
		}
