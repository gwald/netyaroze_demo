...
#if 0
	int angle;
	double acose, asine, fangle, atane;
	int rsine, rcose, rtane;
	int rasine, racose, ratane;
	int value;
	int radius = 128;
	int x, y, hypot = 128;
	double xx, yy, hypot2;

	for (angle = 0; angle < 4096; angle += ONE/32)
		{
		x = (hypot * rcos(angle)) >> 12;
		y = (hypot * rsin(angle)) >> 12;
		...
		rasine = rasin3(y, hypot);
		if (rasine == SILLY_VALUE)		
			printf("rasine: silly\n");
		racose = racos3(x, hypot);
		if (racose == SILLY_VALUE)		
			printf("racose: silly\n");
		printf("rasin %d\nracos %d\nadjusted racos %d\n\n", rasine, racose, (2048-racose) );

		ratane = ratan3(y, x);
		if (ratane == SILLY_VALUE)		
			printf("ratane: silly\n");
		printf("ratan %d\n", ratane);
		#endif
		...
		fangle = ((double)angle) * 3.14159 * 2 / ((double)4096);

		printf2("\n\nfangle: %f\n", fangle);
		printf("in degrees: %d\n\n\n", (int) (fangle * 360 / (2 * 3.14159)) );

		printf2("xx %f yy %f\n\n", xx, yy);
		
		asine = asin(yy/hypot2);
		acose = acos(xx/hypot2);
		printf2("asin %f\n\nacos %f\n\n", asine, acose);
...
void InitialiseTheTunnel (void)
{
	...
	TunnelSectionLength = SQUARE_SIZE * 4;
	TunnelSectionRadius = SQUARE_SIZE;

	RotationOrder = Y_THEN_X;



		// initialise the tunnel to void
	...
	for (i = 0; i < NumberTunnelSections; i++)
		{
		setVECTOR( &TheTunnel[i].splinePoint, 0, 0, (i * TunnelSectionLength) );
		}
...
void PropagateRotationsAlongTunnel (void)
{
	...
	SVECTOR rotationVector;
		...
		// first block: calculate the rotations from one section to the next
	...
	for (i = 0; i < NumberTunnelSections; i++)
		...
		relativeVector.vx = ((worldVector.vx * coord->coord.m[0][0])
						+ (worldVector.vy * coord->coord.m[1][0])
						+ (worldVector.vz * coord->coord.m[2][0])) >> 12;
		relativeVector.vy = ((worldVector.vx * coord->coord.m[0][1])
						+ (worldVector.vy * coord->coord.m[1][1])
						+ (worldVector.vz * coord->coord.m[2][1])) >> 12;
		relativeVector.vz = ((worldVector.vx * coord->coord.m[0][2])
						+ (worldVector.vy * coord->coord.m[1][2])
						+ (worldVector.vz * coord->coord.m[2][2])) >> 12;
		...
		// now that we know the rotation angles, do the rotations
		// to get the new, derived coordinate system
	for (i = 0; i < NumberTunnelSections; i++)
		{
		thisSection = &TheTunnel[i];
		...
		if (i == 0)		  // first section always a straight => no rotation on coord system
			continue;		
		...
		if (thisSection->thetaX == 0
			...
			DeriveNewCoordSystemFromRotation (&TheTunnel[i-1].coordinateData,
					&rotationVector, coord);
			}
...
void CreateTheTunnel (void)
{
	...
	for (i = 0; i < NumberTunnelSections-1; i++)
		...
		for (index = 0; index < NUMBER_SHAPES_PER_SECTION-1; index++)
			{
			...
			SVECTOR* rotation, GsCOORDINATE2* output)
{
	...
	assert( CheckCoordinateSystemIsBase(output) == TRUE);
	assert (RotationOrder == Y_THEN_X);		// only one handled so far
...
#define VECTOR_SIZE_TOLERANCE_LIMIT 25
...
int VectorIsUnitSize (VECTOR* vector)
{
	...
	if (abs(length - ONE) < VECTOR_SIZE_TOLERANCE_LIMIT)
		booleanResult = TRUE;
...
int CheckCoordinateSystemIsBase (GsCOORDINATE2* coord)
{
	...
	if (coord->coord.t[0] != 0
		|| coord->coord.t[1] != 0
		|| coord->coord.t[2] != 0)
			return FALSE;

	if (coord->coord.m[0][0] != ONE
		|| coord->coord.m[1][1] != ONE
		|| coord->coord.m[2][2] != ONE)
			return FALSE;

	if (coord->coord.m[0][1] != 0
		|| coord->coord.m[0][2] != 0
		|| coord->coord.m[1][0] != 0
		|| coord->coord.m[1][2] != 0
		|| coord->coord.m[2][0] != 0
...
void CopyCoordinateSystem (GsCOORDINATE2* from, GsCOORDINATE2* to)
{
	...
	to->coord.t[0] = from->coord.t[0];
	to->coord.t[0] = from->coord.t[0];
	to->coord.t[0] = from->coord.t[0];

	to->coord.m[0][0] = from->coord.m[0][0];
	to->coord.m[0][1] = from->coord.m[0][1];
	to->coord.m[0][2] = from->coord.m[0][2];
	to->coord.m[1][0] = from->coord.m[1][0];
	to->coord.m[1][1] = from->coord.m[1][1];
	to->coord.m[1][2] = from->coord.m[1][2];
	to->coord.m[2][0] = from->coord.m[2][0];
	to->coord.m[2][1] = from->coord.m[2][1];
	to->coord.m[2][2] = from->coord.m[2][2];
