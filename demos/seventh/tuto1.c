...
int NumberOfMiniObjectToDisplay;
...
void main (void)
{
	...
	while (QuitFlag == FALSE)
		...
		for (i = 0; i < NumberOfObjectsToDisplay; i++)
			{
...
void InitialiseAll (void)
{
	...
	dot1 = GetDotProduct (&a, &b);
	dot2 = GetDotProduct (&a, &c);
	dot3 = GetDotProduct (&b, &c);
		
	ang1 = FindAngleBetweenVectors	(&a, &b);
	ang2 = FindAngleBetweenVectors	(&a, &c);
	ang3 = FindAngleBetweenVectors	(&b, &c);
	...
	printf("static vector A:\n\n\n");
	dumpVECTOR(&a);
	for (i = -4096; i < 4096; i += 64)
		{
		setVECTOR ( &b, (r * rcos(i) / ONE), 
					(r * rsin(i) / ONE), 0);
		dot1 = GetDotProduct (&a, &b);
		ang1 = FindAngleBetweenVectors	(&a, &b);

		racose4 = racos4( ONE * b.vx / ONE);
		printf("\n\nangle: %d, cos: %d\n\
				arg to racos %d, racos4: %d\n", 
						i, rcos(i), b.vx, racose4);

		printf("\n\n");
		printf("moving vector B:\n");
		dumpVECTOR(&b);
		printf("dot: %d, ang: %d\n", dot1, ang1);
		printf("\n\n");
		}
...
void SetUpTheShip (void)
{
	...
	TheShip.movementSpeed = 25;		// reduce for momentum
	TheShip.rotationSpeed = 25;	
	//TheShip.movementMomentumFlag = TRUE;
	TheShip.collisionRadius = 25;	// UNTESTED
