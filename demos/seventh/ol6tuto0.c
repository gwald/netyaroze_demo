...
void InitialiseTheTunnel (void)
{
	...
	TunnelSectionRadius = SQUARE_SIZE;
		...
		// initialise the tunnel data to void
	for (i = 0; i < MAX_TUNNEL_SECTIONS; i++)
		...
		// here: generate spline from list of rotation vectors
	CreateFirstSpline();
...
void CreateTheTunnel (void)
{
	...
	for (i = 0; i < NumberTunnelSections-1; i++)
		...
		for (index = 0; index < NUMBER_SHAPES_PER_SECTION-1; index++)
			...
			// now the last poly
		p1 = &thisSection->pointsOnCircle[0];
		p2 = &thisSection->pointsOnCircle[NUMBER_SHAPES_PER_SECTION-1];
		p3 = &nextSection->pointsOnCircle[0];
		p4 = &nextSection->pointsOnCircle[NUMBER_SHAPES_PER_SECTION-1];
