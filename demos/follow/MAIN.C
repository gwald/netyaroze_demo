...
void DealWithMenuControls (void)
{
	...
	framesSinceLastChoice++;
		...
		// start rolling demo after 5 seconds of inactivity
	if (framesSinceLastChoice > (50 * 5))
		{
		MenuScreenOption = CHOOSE_GAME_DEMO;
		ChangeGameState(MODE_MAIN_GAME);
		BreakoutAutoPilotFlag = TRUE;
		//printf("going to main game from menu screen: TIME_OUT\n");
	...
	if (framesSinceLastChoice >= pause)
		...
		switch(MenuScreenOption)
			...
			case CHOOSE_GAME_DIFFICULTY:
				...
				break;
			case CHOOSE_GAME_DEMO:
                if ((pad & PADLright) || (pad & PADLleft) || (pad & PADRdown) || (pad & PADstart))
					{
					ChangeGameState(MODE_MAIN_GAME);
					BreakoutAutoPilotFlag = TRUE;
