...
void main (void)
{
	...
	while (QuitFlag == FALSE)
		...
		FntPrint("cube scale %d %d %d\n",
			TheCubes[0].scalingVector.vx,
			TheCubes[0].scalingVector.vy,
			TheCubes[0].scalingVector.vz);
...
void DealWithControllerPad (void)
{
	...
	if (pad & PADL1)
		...
		// L2 modifier: play with vp and vr
	if (pad & PADL2)
		{
		if (pad & PADRleft)
			{
		...
		// scaling the main cube
	if (pad & PADLleft)
		{
		if (pad & PADRleft)
			{
			TheCubes[0].scalingVector.vx -= 25 * controlSpeed;
			}
		if (pad & PADRright)
			{
			TheCubes[0].scalingVector.vx += 25 * controlSpeed;
		...
		if (pad & PADRup)
			{
			TheCubes[0].scalingVector.vy -= 25 * controlSpeed;
			}
		if (pad & PADRdown)
			{
			TheCubes[0].scalingVector.vy += 25 * controlSpeed;
		...
		if (pad & PADR1)
			{
			TheCubes[0].scalingVector.vz -= 25 * controlSpeed;
			}
		if (pad & PADR2)
			{
			TheCubes[0].scalingVector.vz += 25 * controlSpeed;
