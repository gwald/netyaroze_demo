...
GsSPRITE PurpleNoiseSprite;
	...
	// by definition ...
#define SCREEN_TEXTURE_PIXEL_MODE 16
...
int ScreenResolution;	  // one of the two flag values above
RECT TotalScreenRectangle;		// if just one area
RECT FirstScreenRectangle;		  // first buffer
RECT SecondScreenRectangle;		  // second buffer

int bufferIndex;			// which is active
...
ObjectHandler TheReflectors[MAX_REFLECTORS];

u_long FirstReflectivePolygonAddress, SecondReflectivePolygonAddress;
...
void HandleAReflector (ObjectHandler *object);
...
void InitialiseAll (void)
{
	...
	//printf("InitialiseAll: 444444\n");

	ClearTheCreatedTmds();

	//printf("InitialiseAll: 55555\n");

	SetUpTheReflectors();

	//printf("InitialiseAll: 66666\n");

	InitialiseCubes();

	//printf("InitialiseAll: 777777\n");

	InitialiseDrawingProcesses();

	//printf("InitialiseAll: 88888\n");
}
...
void InitialiseGraphicsSystem (void)
{
	...
	assert(currentMode == MODE_NTSC || currentMode == MODE_PAL);
	printf("\n\n\nVideo Mode is ");
	switch(currentMode)
		{
		case MODE_NTSC: printf("NTSC\n\n\n\n"); break;
		case MODE_PAL: printf("PAL\n\n\n\n"); break;
		default:
			assert(FALSE);
		}
	...
	switch(ScreenResolution)
		...
		case LOW_RES:
			...
			setRECT( &TotalScreenRectangle, -1, -1, -1, -1);
			setRECT( &FirstScreenRectangle, 0, 0, ScreenWidth, ScreenHeight);
			setRECT( &SecondScreenRectangle, 0, ScreenHeight, ScreenWidth, ScreenHeight);
			break;
		case HI_RES:
			switch(currentMode)
				{
			...
			setRECT( &TotalScreenRectangle, 0, 0, ScreenWidth, ScreenHeight);
			setRECT( &FirstScreenRectangle, -1, -1, -1, -1);
			setRECT( &SecondScreenRectangle, -1, -1, -1, -1);
			break;
		default:
			assert(FALSE);
		}

	printf("First then Second screen rectangles\n");
	dumpRECT( &FirstScreenRectangle);
	dumpRECT( &SecondScreenRectangle);
}
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_REFLECTORS; i++)
		{
		InitSingleObject(&TheReflectors[i]);

		BringObjectToLife(&TheReflectors[i], REFLECTOR, 
			CUBE_MODEL_ADDRESS, 0, NONE);

		RegisterObjectIntoObjectArray(&TheReflectors[i]);
		}
	...
	for (i = 0; i < MAX_REFLECTORS; i++)
		{
		TheReflectors[i].alive = FALSE;		// start off dead
		}
...
void CreateMoreLittleCubesAroundLargerOne (void)
{
	...
	// initial positioning: translate and rotate until in right places
	// also create and sort the two reflective polygons
...
void SetUpTheReflectors (void)
{
	...
	int distanceFromCentre = 2000;

	assert(ScreenResolution == LOW_RES);		// can sort hi-res later

	WhichWayToFlip = 0;		// take buffer 0 or 1 


		// create first one: poly whose texture is first buffer
		// ie texture described by FirstScreenRectangle
	copyRECT( &FirstScreenRectangle, &rect);
	...
	****/
	...
	FirstReflectivePolygonAddress = CreateAnotherPolygonTMD (0, 0, 
						tPageID, SCREEN_TEXTURE_PIXEL_MODE, 
						 u0, v0,
						 u1, v1, 
						 u2, v2, 
						 u3, v3,
		...
		// create second one: poly whose texture is second buffer
		// ie texture described by SecondScreenRectangle
	copyRECT( &SecondScreenRectangle, &rect);
	...
	****/
	...
	SecondReflectivePolygonAddress = CreateAnotherPolygonTMD (0, 0, 
						tPageID, SCREEN_TEXTURE_PIXEL_MODE, 
						 u0, v0,
						 u1, v1, 
						 u2, v2, 
		 ...
		 // creating and positioning the reflector objects
	object = &TheReflectors[0];
	object->alive = TRUE;
	setVECTOR( &object->position, -distanceFromCentre, 0, 0);
	setVECTOR( &object->twist, 0, -ONE/4, 0);
	UpdateObjectCoordinates (&object->twist,
							&object->position, &object->velocity,
							&object->coord, &object->matrix);
	LinkObjectHandlerToSingleTMD (&object->handler, 
							FirstReflectivePolygonAddress);



	object = &TheReflectors[1];
	object->alive = TRUE;
	setVECTOR( &object->position, distanceFromCentre, 0, 0);
	setVECTOR( &object->twist, 0, ONE/4, 0);
	UpdateObjectCoordinates (&object->twist,
	...
	object = &TheReflectors[2];
	object->alive = TRUE;
	setVECTOR( &object->position, 0, distanceFromCentre, 0);
	setVECTOR( &object->twist, -ONE/4, 0, 0);
	UpdateObjectCoordinates (&object->twist,
							&object->position, &object->velocity,
							&object->coord, &object->matrix);
	LinkObjectHandlerToSingleTMD (&object->handler, 
	...
	object = &TheReflectors[4];
	object->alive = TRUE;
	setVECTOR( &object->position, 0, 0, distanceFromCentre);
	...
	object = &TheReflectors[5];
	object->alive = TRUE;
	setVECTOR( &object->position, 0, 0, -distanceFromCentre);
	setVECTOR( &object->twist, 0, ONE/2, 0);
	UpdateObjectCoordinates (&object->twist,
							&object->position, &object->velocity,
							&object->coord, &object->matrix);
	LinkObjectHandlerToSingleTMD (&object->handler, 
...
void InitialiseCubes (void)
{
	...
	CreateCube ( &InsideCube, INSIDE_CUBE, 250, VISIBLE_FROM_OUTSIDE,
	...
	setRECT( &singleOutsideCubeTexture,
			...
			Wave2TextureInfo.pw,
			Wave2TextureInfo.ph);
	CreateCube ( &OutsideCube, OUTSIDE_CUBE, 5000, VISIBLE_FROM_INSIDE, 4,
...
void HandleAReflector (ObjectHandler *object)
{
	assert(object->type == REFLECTOR);

		// no movement or rotation
		// must ensure that it gets the right buffer Index to avoid flicker
		// whichWayToFlip:
		// zero: FirstReflectivePolygonAddress when bufferIndex 0,
		// SecondReflectivePolygonAddress when bufferIndex 1;
		// first, the other way round

	switch(bufferIndex)
		{
		case 0:									  
			switch(WhichWayToFlip)
				{
				case 0:
					LinkObjectHandlerToSingleTMD (&object->handler, 
							FirstReflectivePolygonAddress);
				...
				case 1:
					LinkObjectHandlerToSingleTMD (&object->handler, 
							SecondReflectivePolygonAddress);
					break;
				default:
					assert(FALSE);
				}
...
void SetUpDrawingProcesses (void)
{
	...
	setRECT( &clipArea, WaveTextureInfo.px, WaveTextureInfo.py, 
		WaveTextureInfo.pw, WaveTextureInfo.ph);
	offsets[0] = WaveTextureInfo.px;
	offsets[1] = WaveTextureInfo.py;
	dumpRECT( &clipArea);
