...
long CurrentVideoMode;

extern DISPENV GsDISPENV;

extern int DesiredVideoMode;
...
main()
{
	...
	printf("desired video mode: %d\n", DesiredVideoMode);


		// use the desired video mode from included file 'vid_mode.h'
	if (DesiredVideoMode == MODE_PAL)
		{
		printf("setting PAL mode\n");
		GsDISPENV.screen.y = PAL_SCREEN_Y_OFFSET;
		GsDISPENV.screen.h = PAL_SCREEN_HEIGHT;
		SetVideoMode(MODE_PAL);
		ScreenWidth = 320;
		ScreenHeight = PAL_SCREEN_HEIGHT;
		}
	else if (DesiredVideoMode == MODE_NTSC)
		{
		printf("setting NTSC mode\n");
		GsDISPENV.screen.y = NTSC_SCREEN_Y_OFFSET;
		GsDISPENV.screen.h = PAL_SCREEN_HEIGHT;
		SetVideoMode(MODE_NTSC);
		ScreenWidth = 320;
		ScreenHeight = NTSC_SCREEN_HEIGHT;
		}
	else
		{
		printf("invalid video mode set in vid_mode.h\n");
		exit(1);
		}
	...
	if (CurrentVideoMode == MODE_PAL) 
		{
		printf("new VideoMode is PAL\n");
		}
	else if (CurrentVideoMode == MODE_NTSC) 
		{
		printf("new VideoMode is NTSC\n");
		}
	else
		{
		printf("invalid video mode\n");
	...
	GsInitGraph(ScreenWidth, ScreenHeight, 4, 0, 0);	
	...
	if (DesiredVideoMode == MODE_PAL)
		...
		printf("setting NTSC mode\n");
		GsDISPENV.screen.y = NTSC_SCREEN_Y_OFFSET;
		GsDISPENV.screen.h = NTSC_SCREEN_HEIGHT;
		}
	else
		{
		printf("invalid video mode\n");
...
static long pad_read(long n)
{
	...
	if (frameNumber % 60 == 0)
		{
		printf("GsDISPENV.screen.y: %d\n", GsDISPENV.screen.y);
		printf("GsDISPENV.screen.h: %d\n", GsDISPENV.screen.h);
		}
	...
	if (padd & PADR2 && (frameNumber % 5 == 0))
		GsDISPENV.screen.y++;
	if (padd & PADR1 && (frameNumber % 5 == 0))
		{
		GsDISPENV.screen.y--;
		if (GsDISPENV.screen.y < 0)
			GsDISPENV.screen.y = 0;
		}
	if (padd & PADRup && (frameNumber % 5 == 0))
		GsDISPENV.screen.h++;
	if (padd & PADRdown && (frameNumber % 5 == 0))
		{
		GsDISPENV.screen.h--;
		if (GsDISPENV.screen.h < 0)
			GsDISPENV.screen.h = 0;
		}		
