...
void HandleSpecialMovement (ObjectHandler* object)
{
	...
	switch (object->specialMovement)	
		...
		case SECOND_SIMPLE_PATH:
				// if at corner of geometric cube, flip around
			if ( (mod(object->position.vx) == HALF_RECTANGLE)
				&& (mod(object->position.vy) == HALF_RECTANGLE)
				&& (mod(object->position.vz) == HALF_RECTANGLE) )
					{
					...
					if (object->position.vx > 0)
						object->velocity.vx = -(mod(object->velocity.vx));
					if (object->position.vy > 0)
						object->velocity.vy = -(mod(object->velocity.vy));
					if (object->position.vz > 0)				
						object->velocity.vz = -(mod(object->velocity.vz));
					}
			else			// not at corner: adjust velocity
				{
				#if 0		// NOT ADJUSTING USING DOUBLE: too slow
				int distanceFromCentre;
				double speedMultiplier;
					
				distanceFromCentre 
					= SQUARE_ROUTE_OF_THREE * object->velocity.vx;
				// ensure sign is +ve
				distanceFromCentre = mod(distanceFromCentre);

					// move faster as approach the centre
				if (distanceFromCentre > 0)
					{
					speedMultiplier = CORNER_DISTANCE / distanceFromCentre;

					object->velocity.vx *= speedMultiplier;
					object->velocity.vy *= speedMultiplier;
					object->velocity.vz *= speedMultiplier;
					}
				#endif
				}	
			break;
...
void SetUpSolarSystem (void)
{
	...
	for (i = 0; i < 4; i++)
		...
		TheShips[i].position.vx = 0;
		TheShips[i].position.vy = 0;
		TheShips[i].position.vz = 0;
