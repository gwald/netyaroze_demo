		...
		for (i = 0; i < MAX_OBJECTS; i++) 
			...
			if (*pointer != NULL)
				...
				if ( (*pointer)->alive == TRUE)
					{
					object = *pointer;
					if (object == &TheShips[4])
					{
					printf("5th ship in drawing part!\n");
					}
...
void HandleAllObjects (void)
{
	...
	printf("fifth ship position: %d, %d, %d\n",
		TheShips[4].position.vx, TheShips[4].position.vy, TheShips[4].position.vz);
...
void SetUpSolarSystem (void)
{
	...
	BringObjectToLife ( &TheShips[4], SHIP, SHUTTLE_MODEL_ADDRESS, 0, NONE);
	TheShips[4].position.vx = 0;
	TheShips[4].position.vy = 0;
	TheShips[4].position.vz = 0;
	

			// make it larger to be visible
	//SetObjectScaling( &TheShips[4], 64 * ONE, 64 * ONE, 64 * ONE);

		 				
	RegisterObjectIntoObjectArray( &TheShips[4]);
}
