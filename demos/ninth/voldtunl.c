										...
										int whichSection, VECTOR* offset)
{
	...
	// ie we assume ship is looking forwards
...
void BringAliveLocalTunnelAroundShip (ObjectHandler* object)
{
	...
	assert(ValidID(objectSectionID));
		...
		// make this section and some of those ahead visible;
		// subdivide a few
	for (i = 0; i < NUMBER_SECTIONS_ALIVE; i++)
		{
		sectionID = objectSectionID + i;
		...
		for (j = 0; j < NUMBER_SHAPES_PER_SECTION; j++)
			...
			if (i < SUBDIVISION_LOOKAHEAD)			// turn nonexisting subdivision on
				{
...
void HandleShipsMovementInTunnel (ObjectHandler* object)
{
	...
	VECTOR *splinePoint;
	VECTOR worldPos, relativePosition;
	int legalDistanceFromSectionCentralLine;
	int x, y;
	int shipDirection;		// forwards or backwards
	int viewSection, newViewSection;
	...
	nextSection = FindClosestSectionToPoint( &newPosition, object->tunnelSection);
		...
		// is ship going forwards or backwards ?
	shipDirection = FindWhichWayShipPointsInSection(object, thisSection);
	
#if 0				 // very crude collision detection
					// proper way of doing coming soon
...
#if 0
	viewSection = FindSectionViewpointIsIn( &object->position,
								object->tunnelSection);
	newViewSection = FindSectionViewpointIsIn( &newPosition,
								nextSection->id);
	if (viewSection != newViewSection)
		{
		UpdateTunnelSubdivision(viewSection, newViewSection);
		}
#endif
	

		// what happens when ship crosses from one section
		// unto the next ...
	if (nextSection->id != object->tunnelSection)
		{
			// update the tunnel
		UpdateTunnelPolygonStatus(object->tunnelSection,
							nextSection->id, shipDirection);
...
int FindSectionViewpointIsIn (VECTOR* point, int sectionID)
{
	...
	view.vx += (TheShip.coord.coord.m[0][0] * TheView.vpz) >> 12;
	view.vy += (TheShip.coord.coord.m[1][0] * TheView.vpz) >> 12;
	view.vz += (TheShip.coord.coord.m[2][0] * TheView.vpz) >> 12;
...
int FindWhichWayShipPointsInSection (ObjectHandler* object, TunnelSection* section)
{
	int direction;
	VECTOR zAxis, relativeZaxis;

		// express ship z axis in section coordinates
		// to find out which way ship is going round tunnel
	setVECTOR( &zAxis, 
			object->coord.coord.m[0][2],
			object->coord.coord.m[1][2],
			object->coord.coord.m[2][2]); 
	ExpressSuperPointInSub ( &zAxis, 
				&section->coordinateData, &relativeZaxis);

		// reverse vector since our ship TMD points wrong way
	relativeZaxis.vx = -relativeZaxis.vx;
	relativeZaxis.vy = -relativeZaxis.vy;
	relativeZaxis.vz = -relativeZaxis.vz;

	if (relativeZaxis.vz < 0)
		direction = BACKWARDS;
	else
		direction = FORWARDS; 
			...
			int* distance, int* direction)
{
	...
	simpleDist = abs(first - second);

	if (simpleDist >= NumberTunnelSections/2)
		{
		}
	else
		{
	...
	// ship starts move in first section and moves to the next;
	// make polygons alive and undead correspondingly;

	// at any one time, NUMBER_SECTIONS_ALIVE sections are alive,
	// and SUBDIVISION_LOOKAHEAD sections have polygons subdivided


void UpdateTunnelPolygonStatus (int firstID, int nextID, int direction)
{
	int sectionID;
	TunnelSection* section;
	int polygonID;
	int numberToChangeLiving;		  // from alive to undead, and vica versa
	int numberToChangeSubdivision;		
	int i, j;
	...
	for (i = 0; i < numberToChangeLiving; i++)
		...
		switch (direction)
			{
			case FORWARDS:	
				sectionID = firstID + NUMBER_SECTIONS_ALIVE + i;  
				break;
			case BACKWARDS:	
				sectionID = firstID - NUMBER_SECTIONS_ALIVE - i;
				break;
	...
	numberToChangeSubdivision 
		= min(numberToChangeLiving, SUBDIVISION_LOOKAHEAD);
	printf("num for subdiv: %d\n", numberToChangeSubdivision);

		// make some no longer divided (since no longer visible)
		// basically a few sections forward from firstID
	for (i = 0; i < numberToChangeSubdivision; i++)
		{
		switch (direction)
			{
			case FORWARDS:	
		...
		// make some divided (since now visible)
		// basically a few sections forward from nextID 
	for (i = 0; i < numberToChangeSubdivision; i++)
		{
		switch (direction)
			{
		...
		for (j = 0; j < NUMBER_SHAPES_PER_SECTION; j++)
			...
			SetMiniObjectSubdivision( &ThePolygons[polygonID], 
				SUBDIVISION_EXTENT);
			}		
		}
}
