...
void InitialiseTrack (void)
{
	...
	NumberOfSections = 2;	// make sure we get first one correct first
	...
	for (i = 0; i < NumberOfSections-1; i++)
		{
		SectionAngles[i].thetaX = 0;		// 45 degrees
		SectionAngles[i].thetaY = ONE/8;		
...
void CreateTheTrackObjects (void)
{
	...
	for (i = 0; i < NumberOfSections-1; i++)
		...
		if (SectionAngles[i].thetaX != 0 
			...
			movementForward.vz = SectionLength;//2;	  // SHOULD BE / 2: just TESTING
			...
			movementForward.vx = 0;
			movementForward.vy = 0;
			movementForward.vz = SectionLength * 2;// SHOULD BE * 1: just TESTING
...
void InitialiseObjects (void)
{
	...
	PlayersShip.position.vz = 5 * FACT;
