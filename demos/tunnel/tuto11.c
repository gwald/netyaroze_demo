...
void main (void)
{
	...
	while(1)
		...
		//printf("6\n");
			...
			{
			FntPrint("dir: %d %d %d\n", 
				TheLights[0].vx, TheLights[0].vy, TheLights[0].vz);
			FntPrint("rgb: %d %d %d\n", TheLights[0].r, TheLights[0].g, TheLights[0].b); 
			}
#endif
...
void HandleEnemyClass (void)
{
	...
	if (frameNumber % RobotFrequency == 0)
		{
		CreateRobot();
		}
...
int NumberOfEnemiesLeft (void)
{
	int numberTieFighters, numberSingleAttackers, numberInsectShips, numberRobot;
	...
	numberRobot = (TheRobot.alive == TRUE) ? 1 : 0;

	return (numberTieFighters + numberSingleAttackers + numberInsectShips + numberRobot);
}
...
void HandleLevelUpdating (void)
{
	...
	SpecialObjectLikelihood = 350 + (levelAboveOne * 25);
}
	...
	for (i = 0; i < MAX_SPECIAL_OBJECTS; i++)
		...
		if (SpecialObjects[i].alive == TRUE && SpecialObjects[i].actionState != HOLD_STILL)
			...
			if (ObjectsVeryClose(&SpecialObjects[i], &PlayersShip))
				...
				SpecialObjects[i].scaleX *= 2;
				SpecialObjects[i].scaleY *= 2;
