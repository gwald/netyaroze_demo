...
void HandleTieFighterClass (void);
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_TIE_FIGHTERS; i++)
		...
		TieFighters[i].initialHealth = 4;
		TieFighters[i].currentHealth = 4;
...
void HandleACube (ObjectHandler* object)
{
	...
	if (object->position.vx <= LeastX || object->position.vx >= MostX
		|| object->position.vy <= LeastY || object->position.vy >= MostY
		|| object->position.vx <= LeastZ || object->position.vz >= MostZ)
		{
		object->alive = FALSE;
		object->lifeTime = 0;
		}
...
#define WAITING_DISTANCE (3 * SQUARE_SIZE / 2)
...
void HandleSingleShooter (ObjectHandler* object)
{
	...
	if (object->coord.coord.t[2] >= WAITING_DISTANCE)
		{
		object->coord.coord.t[2] -= object->movementSpeed;
		}
...
#if 1	 // OLD		 -- CHANGE: move away from player
		// jiggle frequency: every 12 frames (every frame too jittery)
		if (object->lifeTime % 12 == 0)
			{
			whichJiggle = rand() % 4;
		...
		switch(whichJiggle)
			{
			case 0:
				object->coord.coord.t[0] -= object->movementSpeed; 
				break;
			case 1:
				object->coord.coord.t[0] += object->movementSpeed;
				break;
			case 2:
				object->coord.coord.t[1] -= object->movementSpeed;
				break;
			case 3:
				object->coord.coord.t[1] += object->movementSpeed;
				break;
		...
		KeepWithinRange(object->coord.coord.t[0], LeastX, MostX);
		KeepWithinRange(object->coord.coord.t[1], LeastY, MostY);
		object->coord.flg = 0;

		
		NormaliseVector(&firingDirection);
...
void CreateSomeTieFighters (void)
{
	...
	waveType = rand() % NUMBER_WAVE_TYPES;
...
void CreateWaveOfTieFighters (void)
{
	...
	for (i = 0; i < NUMBER_TIE_FIGHTERS_PER_WAVE; i++)
		...
		fighter->currentHealth = 1;		// only 1 for those in waves
...
{
	int id;
	ObjectHandler* shooter;

	id = FindNextFreeTieFighterId();
	if (id == -1)
		return;

	shooter = &TieFighters[id];

	ReviveObject(shooter);

	shooter->specialMovement = SINGLE_SHOOTER;
	shooter->lifeTime = 0;
	
	shooter->coord.coord.t[0] = 0;
	shooter->coord.coord.t[1] = 0;
	shooter->coord.coord.t[2] = 5 * SQUARE_SIZE;
}
...
void ObjectFiresAShot (ObjectHandler* object, VECTOR* direction)
{
	...
	if (bulletID == -1)		// no free shots
		...
		{
		//printf("object type %d firing a shot\n", object->type);

		TheBullets[bulletID].alive = TRUE;
		TheBullets[bulletID].lifeTime = 0;
		TheBullets[bulletID].allegiance = object->allegiance;			
		
			// send in direction of ship at time of fire
		TheBullets[bulletID].velocity.vx 
				= ((object->shotSpeed * normalisedDirection.vx) / 4096);
		TheBullets[bulletID].velocity.vy 
				= ((object->shotSpeed * normalisedDirection.vy) / 4096);;
		TheBullets[bulletID].velocity.vz 
				= ((object->shotSpeed * normalisedDirection.vz) / 4096);
		   		
		TheBullets[bulletID].movementMomentumFlag = TRUE; 
		
					// emitted from nose of ship
		TheBullets[bulletID].position.vx = object->coord.coord.t[0];
		TheBullets[bulletID].position.vy = object->coord.coord.t[1];
		TheBullets[bulletID].position.vz = object->coord.coord.t[2];
		}
}
