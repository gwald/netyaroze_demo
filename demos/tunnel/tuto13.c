...
#define SCREEN_STORE_ADDRESS 0x80090000
...
int SpecialObjectLikelihood = 350;
...
int ViewOscillationJustBegun = FALSE;
	...
	// table reflecting frequency/likelihood of special object subtypes

#define NUMBER_SPECIAL_OBJECT_TABLE_ENTRIES 10
int SpecialObjectTypeTable[NUMBER_SPECIAL_OBJECT_TABLE_ENTRIES] 
	= 
	{
	EXTRA_LIFE,
	FIRE_QUICKER,
...
int GetTypeOfNewSpecialObject (void);
...
void HandleSpecialObjectClass (void)
{
	...
	if (frameNumber > 0 && (count == 0) && ((rand() % SpecialObjectLikelihood) == 0) )
		...
		object->subType = GetTypeOfNewSpecialObject();	
...
int GetTypeOfNewSpecialObject (void)
{
	...
	// type = rand() % NUMBER_SPECIAL_OBJECT_SUBTYPES;

	index = rand() % NUMBER_SPECIAL_OBJECT_TABLE_ENTRIES;
	type = SpecialObjectTypeTable[index];
...
void PerformDelayedAction (DelayedAction* action)
{
	switch(action->typeFlag)
		{
		case QUIT_PROGRAM:
			StoreScreen( (u_long*) SCREEN_STORE_ADDRESS, 0, 0, 320, 240);
