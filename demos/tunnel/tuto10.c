...
void HandleSpecialObject (ObjectHandler* object)
{
	...
	assert(object->type == SPECIAL_OBJECT);
		...
		// make it twist a little (1 degrees per frame)
	switch(object->whichWay)
		{
		case CLOCKWISE:			object->sprite.rotate += 1 * 4096;		break;
		case ANTICLOCKWISE:		object->sprite.rotate -= 1 * 4096;		break;
...
void MinorOscillateLighting (void)
{
	...
	TheLights[2].r = 128; TheLights[2].g = 42; TheLights[2].b = 128;
	angle = cyclePoint * (4096 / LIGHTING_CYCLE_TIME);
