...
#define ENEMY_APPEARANCE_RATE 250
...
#define NUMBER_ATTACKER_SUBTYPES 4
	...
	// insect ships
#define NUMBER_INSECT_SHIPS_PER_WAVE 20
...
int SoundOn;
int WhichProgram, WhichTone;
int FrameWhenSoundStarts;
int SoundDuration;
int SoundStatus;
...
void DealWithControllerPad (void)
{
	...
	if (pad & PADR2)		 // sound check
		...
		if (pad & PADLleft)
			{
			TurnOnSound (27, 0, 10, 10);
			}
		if (pad & PADLright)
			{
			TurnOnSound (31, 0, 10, 10);
			}
		if (pad & PADLup)
			{
			TurnOnSound (47, 0, 10, 10);
			}
		if (pad & PADLdown)
			{
			TurnOnSound (53, 0, 10, 10);
			}
		if (pad & PADRleft)
			{
			TurnOnSound (55, 0, 10, 10);
			}
		if (pad & PADRright)
			{
			TurnOnSound (106, 0, 10, 10);
			}
		if (pad & PADRup)
			{
			TurnOnSound (127, 0, 10, 10);
			}
		return;
		}
#endif
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_INSECT_SHIPS; i++)
		...
		InsectShips[i].movementSpeed = 15;
...
void HandleATieFighter (ObjectHandler* object)
{
	...
	switch(object->specialMovement)
		...
		case DARTING_SWITCHING_HELIX:
			...
			#if 0		 // OLD: precompiled circle positions make no difference
			switch(object->whichWay)
				{
				case CLOCKWISE:
...
void HandleEnemyClass (void)
{
	...
	if (frameNumber % ENEMY_APPEARANCE_RATE == 0 && frameNumber > 0)
		{
		if (NumberOfEnemiesLeft() == 0)
			{
			whichEnemyType = 2; //rand() % NUMBER_ENEMY_CLASSES;
...
int NumberOfEnemiesLeft (void)
{
	int numberTieFighters, numberSingleAttackers;
	...
	return (numberTieFighters + numberSingleAttackers);
}
...
void CreateWaveOfTieFighters (void)
{
	...
	angleStep = ONE / NUMBER_TIE_FIGHTERS_PER_WAVE;
	startX = (rand() % 65) - 32;
	startY = (rand() % 18) - 9;
	...
	switch(waveType)
		...
		case DARTING_SWITCHING_HELIX:
			...
			for (i = 0; i < NUMBER_TIE_FIGHTERS_PER_WAVE; i++)
				...
				#if 0			
				if (fighter->whichWay == CLOCKWISE)
					fighter->angle = initialAngle + (i * angleStep);
				else	
					fighter->angle = initialAngle - (i * angleStep);
				#endif
				fighter->angleIncrement = ANGULAR_SPEED;
...
void CreateWaveOfInsectShips (void)
{
	...
	int idList[NUMBER_INSECT_SHIPS_PER_WAVE];
	ObjectHandler* insect; 

  	for (i = 0; i < NUMBER_INSECT_SHIPS_PER_WAVE; i++)
		{
...
void HandleSound (void)
{
	switch(SoundStatus)
		...
		case WAITING_TO_START:
			if (frameNumber == FrameWhenSoundStarts)
				{
					// turn on
				voice1 = SsUtKeyOn(vab0, WhichProgram, WhichTone, 60, 0, 127, 127);
				SoundStatus = PLAYING;
				}	
			break;
		case PLAYING:
			if (frameNumber == FrameWhenSoundStarts + SoundDuration)
				{
					// turn off
				SsUtKeyOff(voice1, vab0, WhichProgram, WhichTone, 60);
				SoundStatus = OFF;
				}
			break;
		default:
	...
	else if (frameNumber == FrameWhenSoundStarts + SoundDuration)
		{
			// turn off
		SsUtKeyOff(voice1, vab0, WhichProgram, WhichTone, 60);
		}
...
void TurnOnSound (int whichProgram, int whichTone, int delayFromNow, int howLong)
{
	...
	assert(whichTone >= 0);
		...
		// CANNOT start one sound while another in progress
		// -- easy to remove this restriction, but NOT DONE YET
	if (SoundStatus != OFF)
		return;
	...
	FrameWhenSoundStarts = frameNumber + delayFromNow;
	SoundDuration = howLong;

	SoundStatus = WAITING_TO_START;
}
