...
#define VB0_ADDR (unsigned char*)0x80160000L	  // 16
...
void HandleAllObjects (void)
{
	...
	printf("HandleAllObjects: 0\n");

	CheckCollisions();

	printf("HandleAllObjects: 1\n");

	HandleSphereClass();

	printf("HandleAllObjects: 2\n");
	...
	for (i = 0; i < MAX_OBJECTS; i++)
		...
		if (ObjectArray[i] != NULL)
			...
			if (ObjectArray[i]->alive == TRUE)
				{
				printf("HandleAllObjects: 3\n");
				object = ObjectArray[i];
				printf("HandleAllObjects: 4\n");
				object->lifeTime++;
				printf("HandleAllObjects: 5\n");
