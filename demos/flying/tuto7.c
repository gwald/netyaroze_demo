...
void main( void )
{
	...
	while(1)
		...
		GsSortSprite( &Background, &Wot[side], (1<<OT_LENGTH)-1);
...
void DealWithControllerPad (void)
{
	...
	switch(MainMode)
		...
		// select and R1 and R2: step frame by frame
...
void InitialiseAll (void)
{
	...
	InitGsSprite( &Background);

	Background.attribute = 0;
	Background.tpage = GetTPage(0, 0, 640, 256);
	Background.x = -SCREEN_WIDTH/2;
	Background.y = -SCREEN_HEIGHT/2;
	Background.w = 256;
	Background.h = 256;
	Background.scalex = ONE * 320/256;
	Background.cx = 0;
	Background.cy = 481;
	Background.u = 0;
	Background.v = 0;
	Background.r = Background.g = Background.b = 128;
			...
			if (ObjectArray[i]->alive == TRUE)
				...
				switch(object->type)
					...
					case ENEMY:
						...
						break;
					case SQUARE:										
						if (abs(object->position.vx - x) < (SimpleClipDistance * FACT)
							&& abs(object->position.vz - z) < (SimpleClipDistance * FACT))
							{
