...
// nearly there, but it has trashed some textures/models
// of the four single-sounds, all work except for 'aah'

// will redo when Jason available   	  
...
short se_voice1, se_voice2, se_voice3, se_voice4;

#define VH_NAME "\\DATA\\SOUND\\STD0.VH;1"
#define VB_NAME "\\DATA\\SOUND\\STD0.VB;1"
#if 1			// these work, but trash floor TMD/texture
				// when using std0.vh and .vb from Yaroze disc
#define VH0_ADDR (unsigned char*)0x80101000L	  // 18
#define VB0_ADDR (unsigned char*)0x80170000L	  // 16
#endif
#if 0
#define VH0_ADDR (unsigned char*)0x80101000L	  
#define VB0_ADDR (unsigned char*)0x80110000L
#endif
...
int frameWhenSoundStarts = -1;
int whichShortSound = 0;   // NONE


enum {HAND_CLAP = 1, CYMBAL, AAH, BELL};
...
void DealWithSound (void);

void CreateSingleSound (int id);
...
void DealWithControllerPad (void)
{
	...
	if (pad & PADstart)
		...
		// single sounds: all work except for 'aah'
	if (pad & PADselect)
		{
		if (pad & PADLleft)
			{
			frameWhenSoundStarts = frameNumber + 1;
			whichShortSound = HAND_CLAP;
			printf("trying to make HAND_CLAP sound\n");
			}
		if (pad & PADLright)
			{
			frameWhenSoundStarts = frameNumber + 1;
			whichShortSound = CYMBAL;
			printf("trying to make CYMBAL sound\n");
			}
		if (pad & PADLup)
			{
			frameWhenSoundStarts = frameNumber + 1;
			whichShortSound = AAH;
			printf("trying to make AAH sound\n");
			}
		if (pad & PADLdown)
			{
			frameWhenSoundStarts = frameNumber + 1;
			whichShortSound = BELL;
			printf("trying to make BELL sound\n");
			}
		}
}
...
void HandleAllObjects (void)
{
	...
	//printf("HandleAllObjects 01\n");
	x = PlayersShip.position.vx;
	y = PlayersShip.position.vy;
	z = PlayersShip.position.vz;
	//printf("HandleAllObjects 02\n");

	CheckCollisions();
	//printf("HandleAllObjects 03\n");

	//HandleSphereClass();	// no more spheres

	for (i = 0; i < MAX_OBJECTS; i++)
		{
		//printf("HandleAllObjects 04\n");
		//printf("pointer: %d\n", ((u_long)ObjectArray[i]) );
		if (ObjectArray[i] != NULL)
			{
			//printf("HandleAllObjects 05\n");
			if (ObjectArray[i]->alive == TRUE)
				{
				//printf("HandleAllObjects 06\n");
				object = ObjectArray[i];
				//printf("HandleAllObjects 07\n");
				object->lifeTime++;
				//printf("HandleAllObjects 08\n");

				//printf("HandleAllObjects: type is %d\n", object->type);			 
				switch(object->type)
					{
					case PLAYER:
					...
					case SPHERE:
						...
						break;
					case SQUARE:		
							// if very close to view, sort subdivision		
						DealWithSquareAttribute(object);
						break;
					default:
						printf("bad type is %d\n", object->type);
						assert(FALSE);
					}
				}
			}
...
// NOTE: most/all of the sound-handling code
	// directly nicked from the SCEI sample "novideo"

void InitialiseSound (void)
{
	CdlFILE vhfile, vbfile;			// loaded from Yaroze CD
	u_long * check_head;
	u_long tempo_raw;
	u_char* temptr;
	int cnt;
	...
	printf("after CdSearchFile for vhfile\n");
	printf("vhfile.size: 0x%lX\n", vhfile.size);
	...
	printf("after CdReadFile for vhfile\n");
	...
	printf("after CdSearchFile for vbfile\n");
	printf("vbfile.size: 0x%lX\n", vbfile.size);
	...
	printf("After CdReadFile for vb file\n");
	...
	printf("just before VAB transfer\n");
...
void DealWithSound (void)
{
	if (frameNumber == frameWhenSoundStarts)
		{
			// turn it on
		switch(whichShortSound)
			{
			case HAND_CLAP:
	...
	else if (frameNumber == frameWhenSoundStarts+1)
		{
			// turn it off
		switch(whichShortSound)
			{
...
void CreateSingleSound (int id)
{
	...
	printf("trying to make single sound id %d\n", id);
}
