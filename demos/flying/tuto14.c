...
#define FIRE_2_TEXTURE_ADDRESS 0x800bb000
GsIMAGE Fire2TextureInfo;
...
u_short SortSpriteObjectPosition (ObjectHandler* object)
{
	...
	visualX = object->width * ProjectionDistance / screen.vz;
	visualY = object->height * ProjectionDistance / screen.vz;
	...
	object->sprite.scalex = ONE * visualX / object->width;
	object->sprite.scaley = ONE * visualY / object->height;
	...
	if (screen.vz < ProjectionDistance/2)
		...
		object->sprite.attribute = 0;		// ensure 31st bit is OFF
	...
	// if they're close to players ship,
...
void InitialiseAll (void)
{
	...
	ProperInitialiseTexture(FIRE_2_TEXTURE_ADDRESS, &Fire2TextureInfo);
		// star fields
	InitialiseBackgroundSprite( &Background1);
	InitialiseBackgroundSprite( &Background2);
		// 'circle' scanner
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_BULLETS; i++)
		...
		TheBullets[i].width = 24;
		TheBullets[i].height = 24;
	...
	for (i = 0; i < MAX_CUBES; i++)
		...
		TheCubes[i].imageInfo = &FireTextureInfo;
		TheCubes[i].width = 24;
		TheCubes[i].height = 24;
	...
	for (i = 0; i < MAX_BULLETS; i++)
		...
		//SortFireSprite( &TheBullets[i]);
		}
	for (i = 0; i < MAX_CUBES; i++)
		{
		TheCubes[i].alive = FALSE;		// start off dead
		//SortFireSprite2( &TheCubes[i]);
		}
	for (i = 0; i < MAX_SPHERES; i++)
		{
...
void SortFireSprite (ObjectHandler* object)
{
	object->sprite.tpage = GetTPage(0, 0, 960, 0);
	object->sprite.u = 0;
	object->sprite.v = 84;
	object->sprite.cx = 0;
	object->sprite.cy = 484;
	object->sprite.w = 32;
	object->sprite.h = 32;
}
...
void SortFireSprite2 (ObjectHandler* object)
{
	object->sprite.tpage = GetTPage(1, 0, 960, 0);	  // 8-bit texture
	object->sprite.attribute |= 1 << 24;				// 8-bit flag
	object->sprite.u = 0;
	object->sprite.v = 200;
	object->sprite.cx = 0;
	object->sprite.cy = 486;
	object->sprite.w = 32;
	object->sprite.h = 32;
}
...
void HandleAllObjects (void)
{
	...
	int i;
	int x, y, z;
	int scaler;				// for explosion objects
			...
			if (ObjectArray[i]->alive == TRUE)
				...
				switch(object->type)
					...
					case BULLET:
						...
						scaler = ONE - (object->lifeTime * 70);
							
						SetObjectScaling (object, scaler, scaler, scaler);
