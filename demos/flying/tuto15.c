...
#define MAX_EXPLOSION_CENTRES 15
ObjectHandler ExplosionCentres[MAX_EXPLOSION_CENTRES];
...
void SortCubeClutDimming (ObjectHandler* object);
...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_EXPLOSION_CENTRES; i++)
		{
		InitSingleObject(&ExplosionCentres[i]);

		BringObjectToLife(&ExplosionCentres[i], EXPLOSION_CENTRE, 
			CUBE_MODEL_ADDRESS, 0, NONE);

		ExplosionCentres[i].displayFlag = SPRITE;
		ExplosionCentres[i].imageInfo = &MultipleTextureInfo;

		RegisterObjectIntoObjectArray(&ExplosionCentres[i]);
		}
	//printf("4\n");
	...
	for (i = 0; i < MAX_EXPLOSION_CENTRES; i++)
		{
		ExplosionCentres[i].alive = FALSE;	   // start off dead

			// don't want to take all of this TIM, only a small portion
		ExplosionCentres[i].sprite.w /= 4;
		ExplosionCentres[i].sprite.h /= 4;
		ExplosionCentres[i].sprite.u += 32;
		ExplosionCentres[i].sprite.v += 64;					  				   

			// start off at random rotation
		ExplosionCentres[i].sprite.rotate = (rand() % 360) * 4096;
		}
	printf("8\n");
}
				...
				switch(object->type)
					...
					case CUBE:
						...
						if (object->rotationMomentumFlag == FALSE)
							...
							// make it twist a little (1 degree per frame)
						object->sprite.rotate += 4096;
...
#if 0			// NOT YET WORKING PROPERLY: need to reload after death
							// reduce CLUT over time
						if (object->lifeTime % 5 == 0)
							{
							SortCubeClutDimming(object);
							}
#endif
...
u_short originalCLUT[16], newCLUT[16];	
...
void SortCubeClutDimming (ObjectHandler* object)
{
	...
	setRECT( &rect, MultipleTextureInfo.cx, MultipleTextureInfo.cy,	
				MultipleTextureInfo.cw, MultipleTextureInfo.ch); 

	if (object->lifeTime == 0)
		{
		StoreImage( &rect, (u_long*)originalCLUT);
		DrawSync(0);
		...
		for (i = 0; i < 16; i++)
			...
			if (originalCLUT[i] != 0)
				{
				printf("entry %d non-zero!\n", i);
				printf("rgb: %d %d %d\n", (i&31), ((i>>5)&31), ((i>>10)&31) );
				}
			}
		}
	else if (object->lifeTime == 24)	  // save original back into place
		{
		LoadImage( &rect, (u_long*)originalCLUT);
		DrawSync(0);
		}
	else
		{
		reduction = object->lifeTime / 5;
		for (i = 0; i < 16; i++)
			{
			r = originalCLUT[i] & 31;
			g = (originalCLUT[i] >> 5) & 31;
			b = (originalCLUT[i] >> 10) & 31;
			...
			printf("reduced rgb: %d %d %d\n", r, g, b);

			newCLUT[i] = (b << 10) + (g << 5) + r;
...
void ExplodeObject (ObjectHandler* object)
{
	int cubeID, explosionID;
	int whichAxis, sign;
	int i, j;
	
	object->alive = FALSE;

		// create one explosion-centre object
	explosionID = -1;	

		// find next available cube
	for (j = 0; j < MAX_EXPLOSION_CENTRES; j++)
		{
		if (ExplosionCentres[j].alive == FALSE)
			{
			explosionID = j;
			break;
			}
	...
	if (explosionID != -1)		
		{
		ExplosionCentres[explosionID].alive = TRUE;
		ExplosionCentres[explosionID].lifeTime = 0;

			// position at centre of exploding object
		ExplosionCentres[explosionID].position.vx = object->position.vx;
		ExplosionCentres[explosionID].position.vy = object->position.vy;
		ExplosionCentres[explosionID].position.vz = object->position.vz;
		}
