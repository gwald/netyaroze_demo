...
// changes from tuto2:
// bringing in sound features (don't load yet)
// new model and texture -> smoother
...
#define VH0_ADDR (unsigned char*)0x80180000L
...
void main( void )
{
	...
	while(1)
		...
		if (frameNumber % 15 == 0)
			{
			printf("square scaling:\n");
			dumpVECTOR(&scalingVector);
			printf("whichWay: %d\n", whichWay);
			}
		...
		case MOVE_SHIP:
			if (pad & PADR2)
				...
				switch(whichWay)
					{
					case 0:
						scalingVector.vx += ONE/100;
						break;
					case 1:
						scalingVector.vy += ONE/100;
						break;
					case 2:
						scalingVector.vz += ONE/100;
						break;
					}
				}
			if (pad & PADR1)
...
void PlayerFiresAShot (void)
{
	...
	if (bulletID == -1)		// no free shots
		...
		frameWhenSoundStarts = frameNumber;
		}
}
...
void InitialiseAll (void)
{
	...
	printf("iseAll: 1\n");
		...
		// very crude: uses current day of compilation
		// should use time:seconds
	{
	int day; 
	char* pointer;
	char dateString[64];

	strcpy(__DATE__, dateString); 

	pointer = &dateString[0];
	pointer += 4;

	day = 10 * ((*pointer) - 48) + (*(pointer+1)-48);
	srand(day);
	}
#endif
...
{	
	...
	if (CollisionWithWalls(position, &realMovement) == FALSE)  
		...
		velocity->vz = 0;
			...
			// if this is player's ship, make collision sound
		if (position->vx == PlayersShip.position.vx
			&& position->vy == PlayersShip.position.vy
			&& position->vz == PlayersShip.position.vz)
				{
...
void PositionSomewhereNotInAWall (ObjectHandler* object)
{
	...
	object->twist.vx = rand() % ONE;
	object->twist.vy = rand() % ONE;
	object->twist.vz = rand() % ONE;
#endif
...
void HandleLevelTransition (void)
{
	...
	// EXTRA bonus fact about matrices
	// (certainly when manipulated by UpdateObjectCoordinates):
	// not only do m[0][0], m[1][0], m[2][0] represent 
	// object's own x axis in world coordinate terms,
	// and similarly for object's own y and z axis:

	// but m[0][0], m[0][1], m[0][2] is the world x axis
	// in object's coordinate system.
	// This is VERY useful for 3D navigation and tracking,
	// eg when tracking another object, if you express the vector
	// from you to that object in YOUR coordinate system rather than
	// the world's, tracking it is vastly simpler.
