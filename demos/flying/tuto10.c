...
void SortShipSprites (GsOT* ot)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		...
		if ( ((ox * ox) + (oz * oz)) < (ScannerRange * ScannerRange) )  // if in range
			...
			ShipSprites[i].x = -SCREEN_WIDTH/2 + OVAL_CENTRE_X + 8 + xInCircle - 2;
			ShipSprites[i].y = -SCREEN_HEIGHT/2 + OVAL_CENTRE_Y + 19 - zInCircle - 2;
...
void InitialiseOvalSprite (void)
{
	...
	OvalSprite.attribute |= 1<<24;			// 8-bit CLUT
...
void InitialiseShipSprites (void)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		...
		ShipSprites[i].h = 2;
		ShipSprites[i].u = 11;		
		ShipSprites[i].v = 52;	   // 52 plus 0 -> red dot
		ShipSprites[i].cx = 0;
