...
void InitialiseObjects (void)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		{
		TheShips[i].alive = FALSE;		// start off dead
		}
...
void HandleShipClass (void)
{
	...
	numberOfShips = CountNumberOfLivingTypedObjects(ENEMY);

	if (numberOfShips == 0 && ((rand() % SHIP_CREATION_FREQUENCY) == 0) )
		{
		TheShips[0].alive = TRUE;
		TheShips[0].currentHealth = 10;
		printf("count AFTER creation: %d\n", CountNumberOfLivingTypedObjects(ENEMY) );

		PositionSomewhere(&TheShips[0]);

		if (rand() % 2 == 0)
			{
			TheShips[0].strategyFlag = STATIONARY_TRACK;
			printf("creating sitting tracker\n");
			}
		else
			{
			TheShips[0].strategyFlag = SEEKING;
