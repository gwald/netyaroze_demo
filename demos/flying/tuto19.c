...
#define ENEMY_MODEL_ADDRESS 0x800c0000
...
#define WORLD_MAXIMUM_X 2432		   // (WORLD_MAX_X-1) * GRID_SIZE
#define WORLD_MAXIMUM_Y 1152		   // etc
#define WORLD_MAXIMUM_Z 2432


#define HEIGHT_ABOVE_PLANE (GRID_SIZE/2)
...
#define FLAT_BASE 6
#define INSECT_FIGHTER 7
#define TIE_FIGHTER 8
#define LITTLE_PERSON 9


#define NUMBER_OBJECT_TYPES 10
...
#define ENEMY 1
	...
	// healths
int InitialHealthArray[NUMBER_OBJECT_TYPES];
...
#define MAX_SHIPS 6
ObjectHandler ThePrimaryShips[MAX_SHIPS];

	// these are really the things used for explosions, not cubes at all
#define MAX_CUBES 20
...
ObjectHandler MotherShip;

#define MAX_FLAT_BASES 8
ObjectHandler FlatBases[MAX_FLAT_BASES];

#define MAX_INSECT_FIGHTERS 12
ObjectHandler InsectFighters[MAX_INSECT_FIGHTERS];

#define MAX_TIE_FIGHTERS 6
ObjectHandler TieFighters[MAX_TIE_FIGHTERS];



#define MAX_LITTLE_PEOPLE 24
ObjectHandler LittlePeople[MAX_LITTLE_PEOPLE];
...
void HandlePlayer (ObjectHandler* player);

void HandleBullet (ObjectHandler* object);

void HandleCube (ObjectHandler* object);
void SortCubeSpriteDimming (ObjectHandler* object);

void HandleEnemyShip (ObjectHandler* object);

void DealWithSquareAttribute (ObjectHandler* object);
int FindSubdivisionDistanceFromViewMode (void);

void HandleMotherShip (ObjectHandler* object);
void MotherShipCreatesAChild (void);

int FindNextFreeFlatBaseID (void);
int FindNextFreeInsectFighterID (void);
int FindNextFreeTieFighterID (void);


void HandleFlatBase (ObjectHandler* object);
...
void main (void)
{
	...
	while(1)
		...
		//	CheckForEndOfLevel();	
			...
			// make the light sources circle round
		OscillateLighting();		
		...
		FntPrint("enemy 0 at\n%d %d %d\n", 
			ThePrimaryShips[0].position.vx, 
			ThePrimaryShips[0].position.vy, 
			ThePrimaryShips[0].position.vz);	 
...
int FindClipDistanceFromViewMode (void)
{
	...
	switch(ViewMode)
		...
		case VIEW_8:	distance = 3; 	break;
		case VIEW_9:	distance = 3; 	break;
		default:
...
void SortBlipSprites (GsOT* ot)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		{
		if (ThePrimaryShips[i].alive != TRUE)
			continue;

			// position vector to enemy ship from player, in world coord. system
		wx = ThePrimaryShips[i].position.vx - PlayersShip.position.vx;
		wz = ThePrimaryShips[i].position.vz - PlayersShip.position.vz;
...
void EnemyShipFiresAShot (ObjectHandler* enemy)
{
	...
	if (bulletID == -1)		// no free shots
		...
		TheBullets[bulletID].alive = TRUE;
		TheBullets[bulletID].lifeTime = 0;
		TheBullets[bulletID].allegiance = ENEMY_SHIP;			
...
{	
	...
	FntLoad(960, 256);
	FntOpen(64, 32, 128, 100, 0, 512);		  // 64, 32, 256, 200 as first four args before
}
...
void InitialiseObjects (void)
{
	...
	PlayersShip.initialHealth = InitialHealthArray[PLAYER];
	PlayersShip.currentHealth = InitialHealthArray[PLAYER];
	...
	for (i = 0; i < MAX_SHIPS; i++)
		{
		InitSingleObject(&ThePrimaryShips[i]);

		BringObjectToLife(&ThePrimaryShips[i], ENEMY_SHIP, 
			SHIP_MODEL_ADDRESS, 0, NONE);		
				
		ThePrimaryShips[i].initialHealth = InitialHealthArray[ENEMY_SHIP];
		ThePrimaryShips[i].currentHealth = InitialHealthArray[ENEMY_SHIP];
			
		ThePrimaryShips[i].firingRate = 12;
		ThePrimaryShips[i].framesSinceLastFire = 0;
		ThePrimaryShips[i].shotSpeed = 75; 

			// set strategy
		ThePrimaryShips[i].strategyFlag = INACTIVE; //rand() % (NUMBER_ENEMY_STRATEGIES-2);	// none inactive

		InitialiseShipAccordingToStrategy(&ThePrimaryShips[i]);

		PositionSomewhere(&ThePrimaryShips[i]);
		ThePrimaryShips[i].allegiance = ENEMY_SHIP;

		RegisterObjectIntoObjectArray(&ThePrimaryShips[i]);
		}
	//printf("5\n");
	...
	InitSingleObject(&MotherShip);

	BringObjectToLife(&MotherShip, MOTHER_SHIP, 
		ENEMY_MODEL_ADDRESS, 2, NONE);		
				
	MotherShip.initialHealth = InitialHealthArray[MOTHER_SHIP];
	MotherShip.currentHealth = InitialHealthArray[MOTHER_SHIP];
			
	MotherShip.firingRate = 12;
	MotherShip.framesSinceLastFire = 0;
	MotherShip.shotSpeed = 75; 

	PositionSomewhere(&MotherShip);
	MotherShip.allegiance = ENEMY;

	RegisterObjectIntoObjectArray(&MotherShip);
	//printf("5.2\n");
	...
	for (i = 0; i < MAX_FLAT_BASES; i++)
		...
		BringObjectToLife(&FlatBases[i], FLAT_BASE, 
			ENEMY_MODEL_ADDRESS, 8, NONE);		
				
		FlatBases[i].initialHealth = InitialHealthArray[FLAT_BASE];
		FlatBases[i].currentHealth = InitialHealthArray[FLAT_BASE];
			
		FlatBases[i].firingRate = 12;
		FlatBases[i].framesSinceLastFire = 0;
		FlatBases[i].shotSpeed = 75; 

		PositionSomewhere(&FlatBases[i]);
	...
	for (i = 0; i < MAX_INSECT_FIGHTERS; i++)
		{
		InitSingleObject(&InsectFighters[i]);

		BringObjectToLife(&InsectFighters[i], INSECT_FIGHTER, 
			ENEMY_MODEL_ADDRESS, 3, NONE);		
				
		InsectFighters[i].initialHealth = InitialHealthArray[INSECT_FIGHTER];
		InsectFighters[i].currentHealth = InitialHealthArray[INSECT_FIGHTER];
			
		InsectFighters[i].firingRate = 12;
		InsectFighters[i].framesSinceLastFire = 0;
		InsectFighters[i].shotSpeed = 75; 

		PositionSomewhere(&InsectFighters[i]);
		InsectFighters[i].allegiance = ENEMY;

		RegisterObjectIntoObjectArray(&InsectFighters[i]);
		}
	//printf("5.4\n");
	...
	for (i = 0; i < MAX_TIE_FIGHTERS; i++)
		...
		BringObjectToLife(&TieFighters[i], TIE_FIGHTER, 
			ENEMY_MODEL_ADDRESS, 6, NONE);		
				
		TieFighters[i].initialHealth = InitialHealthArray[TIE_FIGHTER];
		TieFighters[i].currentHealth = InitialHealthArray[TIE_FIGHTER];
			
		TieFighters[i].firingRate = 12;
		TieFighters[i].framesSinceLastFire = 0;
		TieFighters[i].shotSpeed = 75; 

		PositionSomewhere(&TieFighters[i]);
	...
	for (i = 0; i < MAX_LITTLE_PEOPLE; i++)
		{
		InitSingleObject(&LittlePeople[i]);

		BringObjectToLife(&LittlePeople[i], LITTLE_PERSON, 
			ENEMY_MODEL_ADDRESS, 0, NONE);		
				
		LittlePeople[i].initialHealth = InitialHealthArray[LITTLE_PERSON];
		LittlePeople[i].currentHealth = InitialHealthArray[LITTLE_PERSON]; 

		PositionSomewhere(&LittlePeople[i]);
		LittlePeople[i].position.vy += 3 * (HEIGHT_ABOVE_PLANE/4);

		LittlePeople[i].allegiance = PLAYER;

		LittlePeople[i].rotate.vx = 3 * ONE/4;
		UpdateObjectCoordinates2(&LittlePeople[i].rotate,  
							&LittlePeople[i].position, &LittlePeople[i].coord);

		RegisterObjectIntoObjectArray(&LittlePeople[i]);
		}
	//printf("5.6\n");
	...
	for (i = 0; i < MAX_SHIPS; i++)
		{
		ThePrimaryShips[i].alive = FALSE;		// start off dead
		}
	for (i = 0; i < MAX_FLAT_BASES; i++)
		{
	...
	for (i = 0; i < MAX_INSECT_FIGHTERS; i++)
		{
		InsectFighters[i].alive = FALSE;		// start off dead
		}
	for (i = 0; i < MAX_TIE_FIGHTERS; i++)
		{
...
void InitialiseHealths (void)
{
 	InitialHealthArray[PLAYER] = 1000;
	InitialHealthArray[BULLET] = 0;
	InitialHealthArray[ENEMY_SHIP] = 15;
	InitialHealthArray[CUBE] = 0;
	InitialHealthArray[SQUARE] = 0;
	InitialHealthArray[MOTHER_SHIP] = 5000;
	InitialHealthArray[FLAT_BASE] = 5;
	InitialHealthArray[INSECT_FIGHTER] = 4;
	InitialHealthArray[TIE_FIGHTER] = 8;
	InitialHealthArray[LITTLE_PERSON] = 1;
}
			...
			if (ObjectArray[i]->alive == TRUE)
				...
				switch(object->type)
					...
					case CUBE:
						...
						break;
					case ENEMY_SHIP:
						HandleEnemyShip(object);
						break;
					case SQUARE:				
						DealWithSquareAttribute(object);
						break;
					case MOTHER_SHIP:	   
						HandleMotherShip(object);
						break;
					case FLAT_BASE:
						HandleFlatBase(object);
						break;
					case INSECT_FIGHTER:
					case TIE_FIGHTER:
...
void HandlePlayer (ObjectHandler* player)
{
	...
	UpdateObjectCoordinates3 (&player->rotate, &player->twist,
		&player->position, &player->velocity,
		&player->coord);
							
	player->rotate.vx += player->twist.vx;
	player->rotate.vy += player->twist.vy;
	player->rotate.vz += player->twist.vz;
						
	SortObjectSize(player);

	KeepWithinRange(player->position.vx, 0, WORLD_MAXIMUM_X);
	//KeepWithinRange(player->position.vy, 0, -WORLD_MAXIMUM_Y);
	KeepWithinRange(player->position.vz, 0, WORLD_MAXIMUM_Z);

		// momentum or not? 
	if (player->movementMomentumFlag == FALSE)
		{
		player->velocity.vx = 0;
		player->velocity.vy = 0;
		player->velocity.vz = 0;
		}
	if (player->rotationMomentumFlag == FALSE)
		{
		player->twist.vx = 0;
		player->twist.vy = 0;
		player->twist.vz = 0;
...
void HandleBullet (ObjectHandler* object)
{
	...
	if (object->lifeTime > 50)
		...
		// if off the plane, kill off
	if (object->position.vx < 0 || object->position.vx >= WORLD_MAXIMUM_X
		|| object->position.vy > 0 || object->position.vy <= -WORLD_MAXIMUM_Y
		|| object->position.vz < 0 || object->position.vz >= WORLD_MAXIMUM_Z)
		{
		object->alive = FALSE;
		object->lifeTime = 0;
		TURN_NTH_BIT_OFF(object->sprite.attribute, 32, 31);
...
void HandleEnemyShip (ObjectHandler* object)
{
	...
	KeepWithinRange(object->position.vx, 0, WORLD_MAXIMUM_X);
	//KeepWithinRange(object->position.vy, 0, -WORLD_MAXIMUM_Y);
	KeepWithinRange(object->position.vz, 0, WORLD_MAXIMUM_Z);
...
#define MOTHER_SHIP_CREATION_FREQUENCY 600

void HandleMotherShip (ObjectHandler* object)
{
	assert(object->type == MOTHER_SHIP);

	UpdateObjectCoordinates(&object->twist, &object->position, 
								&object->velocity, &object->coord, &object->matrix);

	if (frameNumber % MOTHER_SHIP_CREATION_FREQUENCY == 0)
		MotherShipCreatesAChild();
}
...
void MotherShipCreatesAChild (void)
{
	int childType = FLAT_BASE;				// NOT YET    (rand() % 3) + 6;
	int id, angle;
	ObjectHandler* child;
	...
	switch(childType)
		...
		case FLAT_BASE:
			...
			break;
		case INSECT_FIGHTER:
			id = FindNextFreeInsectFighterID();
			if (id == -1)
				return;
			child = &InsectFighters[id];
			break;
		case TIE_FIGHTER:
			id = FindNextFreeTieFighterID();
			if (id == -1)
				return;
			child = &TieFighters[id];
			break;
		default:
			assert(FALSE);
		}
	...
	angle = rand() % 4096;
	child->alive = TRUE;
	child->currentHealth = InitialHealthArray[child->type];
	child->position.vx = MotherShip.position.vx + ((128 * rcos(angle)) >> 12);
	child->position.vy = -HEIGHT_ABOVE_PLANE;
	child->position.vz = MotherShip.position.vx + ((128 * rsin(angle)) >> 12);
...
int FindNextFreeInsectFighterID (void)
{
	...
	for (i = 0; i < MAX_INSECT_FIGHTERS; i++)
		{
		if (InsectFighters[i].alive == FALSE)
			{
			result = i;
			break;
			}
...
#define GUARDING_PERSON 0
#define SEEKING_PERSON 1
#define FINDING_CLOSEST_PERSON 2
...
void HandleFlatBase (ObjectHandler* object)
{
	...
	switch(object->actionState)
		{
		case GUARDING_PERSON:
			break;
		case SEEKING_PERSON:
			break;
		case FINDING_CLOSEST_PERSON:
			break;
		default:
			assert(FALSE);
		}	
...
long* InitialiseModel (long* address)
{
	long *dop;
...
{
	object->position.vx = (rand() % WORLD_MAX_X) * GRID_SIZE;
		// fixed height above plane
	object->position.vy = -HEIGHT_ABOVE_PLANE;
	object->position.vz = (rand() % WORLD_MAX_Z) * GRID_SIZE;
}
...
#define DISTANCE_NEAR_ENOUGH 300
#define X_Z_RATIO 10
...
void HandleStationaryTrackingShipsBehaviour (ObjectHandler* object)
{
	...
	objectVector.vx = -(worldVector.vx * object->coord.coord.m[0][0]
						...
						+ worldVector.vz * object->coord.coord.m[2][0]) / ONE;
	objectVector.vy = (worldVector.vx * object->coord.coord.m[0][1]
						+ worldVector.vy * object->coord.coord.m[1][1]
						+ worldVector.vz * object->coord.coord.m[2][1]) / ONE;
	objectVector.vz = (worldVector.vx * object->coord.coord.m[0][2]
						+ worldVector.vy * object->coord.coord.m[1][2]
						+ worldVector.vz * object->coord.coord.m[2][2]) / ONE;	

	dx = objectVector.vx;
	dz = -objectVector.vz;	// TMD of ship points the wrong way

#if 0
	if (frameNumber % 12 == 0)
		{
		printf("dx and dz: %d, %d\n", dx, dz);
		}
#endif
	...
	if (dx == 0)
		...
		if (dz > 0)		// ahead
			{	
			if (dz < DISTANCE_NEAR_ENOUGH)
				{
				//printf("directly ahead, close enough, trying to fire\n");
				...
				{			 
				// do nowt
				//printf("directly ahead, not near enough\n");
				}
			}
		else		// behind
			{
			//printf("behind me, turning to the right\n");
			object->twist.vy += object->rotationSpeed;
			}
		}
	else if ( (abs(dx)*X_Z_RATIO) <= dz && dz > 0)
		{
		//printf("almost directly ahead\n");
		if (dz < DISTANCE_NEAR_ENOUGH)
				{
				//printf("almost directly ahead, close enough, trying to fire\n");
				if (object->framesSinceLastFire > object->firingRate)			// rapid fire
						{
						EnemyShipFiresAShot(object);
						object->framesSinceLastFire = 0;
				...
				{
				// do nowt
				//printf("almost directly ahead, not near enough\n");
				}
		...
		if (dx > 0)
			{
			//printf("on the right ==> turning right\n");
			object->twist.vy += object->rotationSpeed;
			}
		else
			{
			//printf("on the left ==> turning left\n");
			object->twist.vy -= object->rotationSpeed;
			}
		}
	//printf("\n");
...
int CheckCollisions (void)
{
	...
	for (i = 0; i < MAX_SHIPS; i++)
		...
		if (ThePrimaryShips[i].alive == TRUE)
			{
			if (ObjectsVeryClose(&ThePrimaryShips[i], &PlayersShip))
				{
					// should kill both really
				printf("Player and Enemy collision\n");
				PlayersShip.currentHealth--;
				ThePrimaryShips[i].currentHealth--;
				}
			}
		}
	...
	for (i = 0; i < MAX_BULLETS; i++)
		{
		if (TheBullets[i].alive == TRUE && TheBullets[i].allegiance == ENEMY_SHIP)
			{
		...
		if (ThePrimaryShips[i].alive == TRUE)
			...
			for (j = 0; j < MAX_BULLETS; j++)
				...
				if (TheBullets[j].alive == TRUE && TheBullets[j].allegiance == PLAYER)
					{
					if (ObjectsVeryClose(&TheBullets[j], &ThePrimaryShips[i]))
						{
...
void ResetAll (void)
{
	...
	assert(FALSE);			// needs updating ALOT
	...
	for (i = 0; i < MAX_SHIPS; i++)
		...
		ThePrimaryShips[i].initialHealth = 1;
		ThePrimaryShips[i].currentHealth = 1; 

			// set strategy randomly
		ThePrimaryShips[i].strategyFlag = rand() % (NUMBER_ENEMY_STRATEGIES-1);

		PositionSomewhereNotInAWall(&ThePrimaryShips[i]);
		ThePrimaryShips[i].allegiance = ENEMY_SHIP;

		ThePrimaryShips[i].position.vy += 20;
		}
...
#define SHIP_CREATION_FREQUENCY 120

void HandleShipClass (void)
{
	int numberOfShips;	

	numberOfShips = CountNumberOfLivingTypedObjects(ENEMY_SHIP);

	if (numberOfShips == 0 && ((rand() % SHIP_CREATION_FREQUENCY) == 0) )
		{
		ThePrimaryShips[0].alive = TRUE;
		ThePrimaryShips[0].currentHealth = 10;
		//printf("count AFTER creation: %d\n", CountNumberOfLivingTypedObjects(ENEMY_SHIP) );

		PositionSomewhere(&ThePrimaryShips[0]);

		if (rand() % 2 == 0)
			{
			ThePrimaryShips[0].strategyFlag = STATIONARY_TRACK;
			printf("creating sitting tracker\n");
			}
		else
			{
			ThePrimaryShips[0].strategyFlag = SEEKING;
