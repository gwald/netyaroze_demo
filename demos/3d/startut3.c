...
#define STARS_TEXTURE_ADDRESS 0x80093000
...
#define MAX_Z_VALUE ((1 << OT_LENGTH) - 1)		
...
#define STAR_FIELD_WIDTH SCREEN_WIDTH
#define STAR_FIELD_HEIGHT SCREEN_HEIGHT

#define MAX_STAR_SPRITES 4
GsSPRITE StarSprites[MAX_STAR_SPRITES];
...
void InitStarSprites (void);

void UpdateStarSprites (void);
...
void main (void)
{
	...
	for	(;;)
		...
		if (frameNumber % 20 == 0)
			...
			// columns show object's own axes expressed in world coordinate system
			// rows show world axes expressed in object's own coordinate system
		...
		UpdateStarSprites();
		for (i = 0; i < MAX_STAR_SPRITES; i++)
			{
			GsSortSprite( &StarSprites[i], &Wot[side], MAX_Z_VALUE);
			}
...
int DealWithControllerPad (void)
{
	...
	move = TheShip.movementSpeed * speedMultiplier;
	rotate = TheShip.rotationSpeed * speedMultiplier;

		// moving the ship: forward and back
		// ie object relative movement
		// ie movement only along object's own z axis
	if (pad & PADLup)
		TheShip.velocity.vz -= move;
	if (pad & PADLdown)
		TheShip.velocity.vz += move;
	...
	if (pad & PADR1)
		{
		TheShip.twist.vz += rotate;
		StarRotation +=	rotate * 360;
		return 1;
		}
	if (pad & PADR2)
		{
...
void InitStarSprites (void)
{
	...
	for (i = 0; i < MAX_STAR_SPRITES; i++)
		{
		InitGsSprite( &StarSprites[i]);

		LinkSpriteToImageInfo (&StarSprites[i], &StarsTextureInfo);

		StarSprites[i].mx = STAR_FIELD_WIDTH/2;
		StarSprites[i].my = STAR_FIELD_HEIGHT/2;

		StarSprites[i].scalex = ONE * STAR_FIELD_WIDTH/256;
		StarSprites[i].scaley = ONE * STAR_FIELD_HEIGHT/256;
		}

	StarSprites[0].x = -STAR_FIELD_WIDTH/2;
	StarSprites[0].y = -STAR_FIELD_HEIGHT/2;

	StarSprites[1].x = STAR_FIELD_WIDTH/2;
	StarSprites[1].y = -STAR_FIELD_HEIGHT/2;

	StarSprites[2].x = -STAR_FIELD_WIDTH/2;
	StarSprites[2].y = STAR_FIELD_HEIGHT/2;

	StarSprites[3].x = STAR_FIELD_WIDTH/2;
	StarSprites[3].y = STAR_FIELD_HEIGHT/2;
}
...
void UpdateStarSprites (void)
{
	StarSprites[0].x = StarX - STAR_FIELD_WIDTH/2;
	StarSprites[0].y = StarY - STAR_FIELD_HEIGHT/2;
	StarSprites[0].rotate = StarRotation;

	StarSprites[1].x = StarX + STAR_FIELD_WIDTH/2;
	StarSprites[1].y = StarY - STAR_FIELD_HEIGHT/2;
	StarSprites[1].rotate = StarRotation;

	StarSprites[2].x = StarX - STAR_FIELD_WIDTH/2;
	StarSprites[2].y = StarY + STAR_FIELD_HEIGHT/2;
	StarSprites[2].rotate = StarRotation;

	StarSprites[3].x = StarX + STAR_FIELD_WIDTH/2;
	StarSprites[3].y = StarY + STAR_FIELD_HEIGHT/2;
	StarSprites[3].rotate = StarRotation;
}
