...
#define TILE_TEXTURE_ADDRESS 0x80092000
...
VECTOR squareScaling;
int scalingChanged = TRUE;
...
void main (void)
{
	...
	for	(;;)
		...
		FntPrint("square scaling:\n");
		FntPrint("%d %d %d\n", squareScaling.vx, squareScaling.vy, squareScaling.vz);
...
{	
	...
	setVECTOR( &squareScaling, ONE, ONE, ONE);
...
void HandleAllSquares (void)
{
	...
	for (i = 0; i < MAX_SQUARES; i++)
		...
		if (TheSquares[i].alive == TRUE)
			...
			if (scalingChanged == TRUE)
				{
				UpdateObjectCoordinates (&square->rotate,
							&square->position,
							&square->coord);
				SetObjectScaling (square, squareScaling.vx,
						squareScaling.vy, squareScaling.vz);
				SortObjectSize (square);
