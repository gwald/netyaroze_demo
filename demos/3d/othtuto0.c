...
#include "storescr.h"
	...
	// for better overall organisation, more partitioning into subfiles would be good
	// eg for sound functions, delayed actions, special objects and their effects,
	// also for addresses where data is loaded into main memory
...
#define MULTIPLE_MODEL_ADDRESS 0x80094000
	...
	// where we store screen data in binary TIM format
#define SCREEN_SAVE_ADDRESS 0x800b0000
...
int SpecialObjectLikelihood = 300;
...
void HandleLevelUpdating (void)
{
	...
	EnemyWaveFrequency = 200 - (levelAboveOne * 25);
	KeepWithinRange(EnemyWaveFrequency, 50, 200);
	RobotFrequency = 2000 - (levelAboveOne * 200);
	KeepWithinRange(RobotFrequency, 1000, 2000);
	...
	KeepWithinRange(SingleAttackerMovementSpeed, 5, 25);
	...
	KeepWithinRange(SingleAttackerFiringRate, 8, 24);
	SingleAttackerShotSpeed = 7 + (levelAboveOne * 2);
	KeepWithinRange(SingleAttackerShotSpeed, 7, 20);


	InsectShipMovementSpeed = 18 + (levelAboveOne * 3);
	KeepWithinRange(InsectShipMovementSpeed, 18, 36);
	...
	SpecialObjectLikelihood = 300 - (levelAboveOne * 25);
	KeepWithinRange(SpecialObjectLikelihood, 120, 300);
}
