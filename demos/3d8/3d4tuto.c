...
GsSPRITE Wave3Sprite;

GsIMAGE Wave4TextureInfo;
GsSPRITE Wave4Sprite;

GsIMAGE Wave5TextureInfo;
GsSPRITE Wave5Sprite;
...
void CycleSpriteBrightnesses (void);


void SetSixSpritesRGB (int r, int g, int b);
...
void main (void)
{
	...
	while (QuitFlag == FALSE)
		{		
		FntPrint("~cf00Press Ldown for dynamic textures\n");
		...
		******/
		...
		CycleSpriteBrightnesses();

		// two sprites: firstly both failed to show TIMs properly
		// after moving them in frame buffer, first worked, second still damaged
		// the second GsIMAGE and GsSPRITE were both just as they should have been
		//OrangeTileSprite.x = 80;OrangeTileSprite.y = -80;
		//MultipleSprite.x = 80;MultipleSprite.y = 20;
		//GsSortSprite( &OrangeTileSprite, &Wot[bufferIndex], 0);
		//GsSortSprite( &MultipleSprite, &Wot[bufferIndex], 0);
...
void InitialiseAll (void)
{
	...
	//StarsSprite.scalex = ONE/4;
	//StarsSprite.scaley = ONE/4;
	...
	printf("ORANGE_TILE_TEXTURE_ADDRESS\n\n");
	dumpIMAGE(&OrangeTileTextureInfo);
	dumpSPRITE(&OrangeTileSprite);
	...
	printf("MULTIPLE_TEXTURE_ADDRESS\n\n");
	dumpIMAGE(&MultipleTextureInfo);
	dumpSPRITE(&MultipleSprite);
	...
	LinkSpriteToImageInfo(&Wave3Sprite, &Wave3TextureInfo);

	ProperInitialiseTexture(WAVE_4_TEXTURE_ADDRESS, &Wave4TextureInfo);
	LinkSpriteToImageInfo(&Wave4Sprite, &Wave4TextureInfo);

	ProperInitialiseTexture(WAVE_5_TEXTURE_ADDRESS, &Wave5TextureInfo);
	LinkSpriteToImageInfo(&Wave5Sprite, &Wave5TextureInfo);
...
{
	int cyclePoint;
	int lightEffectPeriod = 270;
	int subCyclePoint;

	cyclePoint = (frameNumber % lightEffectPeriod);

		// r, g, b cycling
	if (cyclePoint < (lightEffectPeriod/3))
		{
		subCyclePoint = cyclePoint;

		if (subCyclePoint == 0)
			{
			SetSixSpritesRGB(128, 128, 128);
			}

		if (subCyclePoint < (lightEffectPeriod/6))
			{
			SetSixSpritesRGB(128 + ((96 * 6 * subCyclePoint) / lightEffectPeriod), 128, 128);
			}
		else
			{
			subCyclePoint -= lightEffectPeriod/6;
			SetSixSpritesRGB(192 - ((96 * 6 * subCyclePoint) / lightEffectPeriod), 128, 128);
			}
		}
	else if (cyclePoint < ((2*lightEffectPeriod)/3))
		{
		subCyclePoint = cyclePoint - (lightEffectPeriod/3);
		...
		if (subCyclePoint < (lightEffectPeriod/6))
			{
			SetSixSpritesRGB(128, 128 + ((96 * 6 * subCyclePoint) / lightEffectPeriod), 128);
			}
		else
			{
			subCyclePoint -= lightEffectPeriod/6;
			SetSixSpritesRGB(128, 192 - ((96 * 6 * subCyclePoint) / lightEffectPeriod), 128);
			}
		}
	else
		{
		subCyclePoint = cyclePoint - ((2*lightEffectPeriod)/3);
		...
		if (subCyclePoint < (lightEffectPeriod/6))
			{
			SetSixSpritesRGB(128, 128, 128 + ((96 * 6 * subCyclePoint) / lightEffectPeriod));
			}
		else
			{
			subCyclePoint -= lightEffectPeriod/6;
			SetSixSpritesRGB(128, 128, 192 - ((96 * 6 * subCyclePoint) / lightEffectPeriod));
			}
		}
}
...
/*****
	if (frameNumber % 5 == 0)
		{
		printf("SetSixSpritesRGB: r %d g %d b %d\n", r, g, b);
		}
*****/
	...
	PurpleNoiseSprite.r = r;
	PurpleNoiseSprite.g = g;
	PurpleNoiseSprite.b = b;

	GreenCircleSprite.r = r;
	GreenCircleSprite.g = g;
	GreenCircleSprite.b = b;

	StarsSprite.r = r;
	StarsSprite.g = g;
	StarsSprite.b = b;

	OrangeTileSprite.r = r;
	OrangeTileSprite.g = g;
	OrangeTileSprite.b = b;
...
void DealWithControllerPad (void)
{
	long pad;
	int controlSpeed;
	static int firstTimeStartFlag = TRUE;
	GsSPRITE *moveSprite;
	...
	if (pad & PADLdown)		// start offscreen drawing
		{
		if (firstTimeStartFlag == TRUE)
			{
			firstTimeStartFlag = FALSE;
	...
	if (pad & PADL2)
		...
		// sort which sprite gets moved: the one on the face we are viewing
	switch(CubeSurfaceViewed)
		{
		case BACK:
			moveSprite = &FireSprite;
			break;
	    case FRONT:
			moveSprite = &PurpleNoiseSprite;
			break;
		case BOTTOM:
			moveSprite = &GreenCircleSprite;
			break;
		case TOP:
			moveSprite = &StarsSprite;
			break;
		case RIGHT:
			moveSprite = &OrangeTileSprite;
			break;
		case LEFT:
			moveSprite = &MultipleSprite;
			break;
		default:	
			assert(FALSE);
		}

		// Lup modifier: move sprite of the face we are looking at
	if (pad & PADLup)
		{
		if (pad & PADRleft)
			{
			moveSprite->x -= controlSpeed;
			}
		if (pad & PADRright)
			{
			moveSprite->x += controlSpeed;
...
void SortPlaneAndAngleViewed (void)
{
	...
	//printf("matrix: ");dumpMATRIX( &ViewCoords.coord);
...
void DoWorkOfDrawProcess (int processID, GsOT *ot)
{
	switch(processID)
	    ...
	    case SECOND:
			...
			break;
		case THIRD:
			GsSortSprite( &GreenCircleSprite, ot, 0);
			break;
		case FOURTH:
			GsSortSprite( &StarsSprite, ot, 0);
			break;
		case FIFTH:
			//GsSortSprite( &OrangeTileSprite, ot, 0);
			GsSortSprite( &FireSprite, ot, 0);
			break;
		case SIXTH:
			//GsSortSprite( &MultipleSprite, ot, 0);
			GsSortSprite( &FireSprite, ot, 0);
			break;
		default:	
			assert(FALSE);
