...
// Author: George Bain
// Date: March 1999 
// Description: Horizontal Split Screen using mutilple OT's
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("\n\n\n\n\n\n\n\n~c009Horizontal Split Screen using mutilple OT's~c999 \n\n");
...
void UpdateScreen( void )
 {
	...
	GsSetOffset(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 4);

	// Set clipping for upper screen
	clip.x = 0;
	clip.y = 0;
	clip.w = SCREEN_WIDTH;
	clip.h = SCREEN_HEIGHT / 2;
	GsSetClip(&clip);
	// Start Drawing upper screen 
	GsDrawOt(&wot[out_buf]);  	 
	

	// Set offset of lower screen 
	GsSetOffset(SCREEN_WIDTH / 2, SCREEN_HEIGHT * 3 / 4); 
