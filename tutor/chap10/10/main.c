...
// Author: George Bain
// Date: March 1999 
// Description: Parallel light sources
...
//--------------------------------------------------------------------------
// Function: main()
// Description: A cube is displayed with 3 parallel light source effecting
//              the color of the cube.  A grey shaded cube is used to 
//              demonstrate the 3 sources.  The ambient lighting is also
//              reduced to give the colors a more darker appearance.  The
//              higher the ambient lighting the more light the colors look.
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Parallel light sources~c999 \n\n");
		 FntPrint("Loc x:(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y:(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z:(%d) \n\n",coord_obj.coord.t[2]);
		 ...
		 FntPrint("Rot z:(%d) \n",sv_obj.vz);
  		 ...
  		 FntPrint("light 0: rgb:(%d,%d,%d)\n",light[0].r,light[0].g,light[0].b);
		 FntPrint("light 1: rgb:(%d,%d,%d)\n",light[1].r,light[1].g,light[1].b);
		 FntPrint("light 2: rgb:(%d,%d,%d)\n",light[2].r,light[2].g,light[2].b);
