...
GsDOBJ2 obj[3];								 // 3D object handler
GsCOORDINATE2 coord_obj[3];					 // 3D object coordinate handler
SVECTOR sv_obj[3];							 // vector for 3D object
...
//--------------------------------------------------------------------------
// Function: main()
// Description: This example demonstrates coordinates systems in 3D.  
//              
//              *Parent Coordinates*   *Child Coordinates*
//              WORLD                  blue sphere              
//              blue sphere			   red sphere
//              red sphere             yellow sphere
//
//  WORLD -> blue sphere -> red sphere -> yellow sphere
//
// Parameters: none
// Returns: int
//--------------------------------------------------------------------------
...
int main( void )
 {
	...
	InitCoordinates(); 	
	InitTMD( &obj[0], &coord_obj[0], (u_long*)OBJECT_ADDR );   // blue
	InitTMD( &obj[1], &coord_obj[1], (u_long*)OBJECT2_ADDR );   // red
	InitTMD( &obj[2], &coord_obj[2], (u_long*)OBJECT3_ADDR );  // yellow
	 
	InitView(); 
    InitLights();   
    InitFog();

    // blue	location in WORLD coordinate system
    coord_obj[0].coord.t[0] = 2800;
	coord_obj[0].coord.t[1] = -1500;
	coord_obj[0].coord.t[2] = 3300;

    // red location from blue coordinate system
	coord_obj[1].coord.t[0] = 0;
	coord_obj[1].coord.t[1] = 0;
	coord_obj[1].coord.t[2] = 2000;

    // yellow location from red coordinate system
	coord_obj[2].coord.t[0] = 0;
	coord_obj[2].coord.t[1] = 0;
	coord_obj[2].coord.t[2] = 4000;
    ...
    while( !DONE )
      {	 		 	     
 		 ...
 		 FntPrint("~c009Coordinate Systems~c999\n \n");
		 FntPrint("Loc x::(%d) \n",coord_obj[0].coord.t[0]);
		 FntPrint("Loc y::(%d) \n",coord_obj[0].coord.t[1]);
		 FntPrint("Loc z::(%d) \n",coord_obj[0].coord.t[2]);
	     ...
	     sv_obj[1].vy = ((sv_obj[1].vy+5)+ONE)&(ONE-1); 
	     sv_obj[2].vy = ((sv_obj[2].vy-25)+ONE)&(ONE-1);	
 
 		 // red around blue
		 UpdateCoordinates(&sv_obj[1],&coord_obj[0]);		 	

         // yellow around red
		 UpdateCoordinates(&sv_obj[2],&coord_obj[1]);
...
void UpdateScreen( void )
 {
	...
	SortObject(&obj[0],  &wot[out_buf], 14-OT_LENGTH);
    SortObject(&obj[1], &wot[out_buf], 14-OT_LENGTH);
	SortObject(&obj[2], &wot[out_buf], 14-OT_LENGTH);
