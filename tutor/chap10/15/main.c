...
//--------------------------------------------------------------------------
// Function: main()
// Description: Loads and displays a gourand shaded cube.
// Parameters: none
// Returns: int
// Notes: RSDTOOL removes information regarding to gourand shading. It's
//        best to add textures then modify the .mat file for gourand shading.
//        Also subdivision must be set to achieve gourand shaded textures!
//--------------------------------------------------------------------------
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Gourand Shading~c999 \n\n");
		 FntPrint("Loc x:(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y:(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z:(%d) \n\n",coord_obj.coord.t[2]);
