...
//--------------------------------------------------------------------------
// Function: main()
// Description: Loads and displays a textured cube.  
// Parameters: none
// Returns: int
// Notes: Don't forget to load the texture into VRAM.
//
//        Texture Cache:
//			  - 2K in size
//            To take advantage of the high-speed texture cache use the 
//            following:	
//
//                  PIXEL MODE   SIZE(WIDTHxHEIGHT) 
//                  - 4bit texture  (64x64) 
//                  - 8bit texture  (64x32)
//                  - 16bit texture (32x32)
//
//  *WIDTH equals rendered width of texture.
//--------------------------------------------------------------------------
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Texture Mapping~c999 \n\n");
		 FntPrint("Loc x:(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y:(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z:(%d) \n\n",coord_obj.coord.t[2]);
