...
// Author: George Bain
// Date: March 1999 
// Description: Semi-transparency Models
...
//--------------------------------------------------------------------------
// Function: main()
// Description: A cube is displayed with the semi-trans attribute set ON.
//              To toggle between semi-trans or normal use the start and
//              select buttons.
// Parameters: none
// Returns: int
// Notes: N/A
...
int main( void )
 {
    ...
    obj.attribute |= (1<<30); // semi-trans	bit set
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Semi-transparency Models~c999 \n\n");
		 FntPrint("Loc x:(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y:(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z:(%d) \n\n",coord_obj.coord.t[2]);
