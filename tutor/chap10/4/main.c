...
// Author: George Bain
// Date: March 1999 
// Description: Parallel and Perspective Projection
...
int projection =0;							 // 3D view projection

//--------------------------------------------------------------------------
// Function: main()
// Description: When setting the projection distance GsSetProjection() is used
//              and is set in "init.c".  The SCRN_Z is defined as 1000 which is 
//              the default value. A larger value gives more of a parallel type 
//              look and smaller gives a perspective look.
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Parallel and Perspective Projection ~c999\n\n");
		 FntPrint("~c900Switch mode using Select and Start~c999\n\n");
         ...
         switch(projection)
		   {
			 case PARALLEL:				
			 	FntPrint("\n~c909Parallel Projection");
			 break;
			 case PERSPECTIVE:
			    FntPrint("\n~c909Perspective Projection");
			 break;
			 default:break;
		   }
...
void ControlObject( void )
 {
     ...
     if( PAD_PRESS(buffer1,PAD_SELECT) )
	   {   
	     projection =1;
         GsSetProjection(SCRN_Z/2);	// perspective   
	   }
     ...
     if( PAD_PRESS(buffer1,PAD_START) )
	   { 	     
	     projection =0;
	     GsSetProjection(SCRN_Z*2);	// parallel		        
	   }
