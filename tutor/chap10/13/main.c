...
//--------------------------------------------------------------------------
// Function: main()
// Description: Scales the screen coordinates from 16-bit to 19-bit.  Check 
//              the reference manual for an explanation.  To toggle between 
//              the two modes use the start and select buttons.
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
...
int main( void )
 {
    ...
    while( !DONE )
      {	 		 	     
 		 FntPrint("~c009Viewing Distance~c999 \n\n");
		 FntPrint("Loc x:(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y:(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z:(%d) \n\n",coord_obj.coord.t[2]);
         ...
         switch(scale_screen)
		   {
			 case 0:
			     FntPrint("~c90916-BIT view distance\nNormal Scaling~c999\n");
			 break;
			 case 1:
			     FntPrint("~c90919-BIT view distance\n1/8 Scaling~c999\n");
			 break;
           }
		 ControlObject(); 	 		 		 
