...
// Author: George Bain
// Date: March 1999 
// Description: Following Camera System 
...
//              in the world and fixed on the 3D object.  Where ever the object
//              is in the 3D world the camera will always be looking at it.
//
//              However since we are using the objects coordinate system (instead
//              of the WORLD) in relation to the viewing coordinate system the
//              viewing point is always in relation to the cube not the WORLD
//              coordinate system. 
//
//              Remember that the view point is the camera lens and the 
...
int main( void )
 {
	...
	// looking from behind cube  
	view.vpx = 150;
	view.vpy = -4100;
	view.vpz = -5700;
	
	// looking at center of cube
    view.vrx = 0;
	view.vry = 0;
	view.vrz = 0;
    ...
    while( !DONE )
      {	 		 	     
 		  FntPrint("~c009Following Camera System~c999\n \n");
		 FntPrint("Loc x::(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y::(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z::(%d) \n",coord_obj.coord.t[2]);
