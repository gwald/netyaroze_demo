...
// Author: George Bain
// Date: March 1999 
// Description: Moving Camera System
...
int main( void )
 {
	...
	// focus on the cube
    view.vrx = coord_obj.coord.t[0];
	view.vry = coord_obj.coord.t[1];
	view.vrz = coord_obj.coord.t[2];


    while( !DONE )
      {	 		 	     
 		  FntPrint("~c009Moving Camera System~c999\n \n");
		 FntPrint("Loc x::(%d) \n",coord_obj.coord.t[0]);
		 FntPrint("Loc y::(%d) \n",coord_obj.coord.t[1]);
		 FntPrint("Loc z::(%d) \n",coord_obj.coord.t[2]);
