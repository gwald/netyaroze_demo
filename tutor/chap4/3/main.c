...
// Author: George Bain
// Date: July 16, 1998
// Description: Chapter 4: Collision Detecting Example 3 - Bounding Spheres
//				This is a case in which the test can fail. 
...
GsSPRITE square, square2;		    // our sprites 

//--------------------------------------------------------------------------
// Function: main()
// Description: Collision Detecting Example 3 - Bounding Spheres
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
    ...
    while( !DONE )
      {
		...
		FntPrint(fnt_id[0], "~c900Collision Detecting Example 3");
		FntPrint(fnt_id[1], "~c900- Bounding Spheres -");
		FntPrint(fnt_id[2], "~c090Collision case in which test fails");
		...
		if( SphereCollision(&square,&square2) )
		  {
...
void InitSquareSprite( void )
 {	 	
	...
	InitSprite( &square,(1<<24),50,100,22,20,tpage,0,109,tim->cx,tim->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
	
   	InitSprite( &square2,(1<<24),150,150,66,14,tpage,0,159,tim->cx,tim->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );

 }// end InitSquareSprite
...
void UpdateScreen( void )
 {
    ...
    GsSortSprite(&square,&world_ordering_table[output_buffer_index],0);        
    GsSortSprite(&square2,&world_ordering_table[output_buffer_index],0);
