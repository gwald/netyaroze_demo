...
// Description: Chapter 4: Collision Detecting Example 4 - Bounding Boxes (Restore)  
...
GsSPRITE box_one, box_two;			// current sprites
GsSPRITE box_one_old, box_two_old;	// previous sprites


												   
//--------------------------------------------------------------------------
// Function: main()
// Description: Collision Detecting Example 4 - Bounding Boxes
// Parameters: none
// Returns: int
    ...
    while( !DONE )
      {
		...
		FntPrint(fnt_id[0], "~c900Collision Detecting Example 4");
		FntPrint(fnt_id[1], "~c900-Bounding Boxes (Restore)-"); 

        // get the previous box coordinates
		box_one_old = box_one; 
		...
		if( BoxCollision(&box_one,&box_two) )
		  {
		    FntPrint(fnt_id[2], "~c990Collision Detected!");

			// restore to previous coordinates
			box_one = box_one_old;	
		  }	
...
void InitBoxOneSprite( void )
 {	 	
	...
	InitSprite( &box_one,(1<<24),0,0,22,20,tpage,0,0,tim->cx,tim->cy,
...
void InitBoxTwoSprite( void )
 {	 	
	...
	tim = ReadTIM((u_long*)TIM_ADDR);    
	tpage = GetTPage(1,0,tim->px,tim->py);		
	InitSprite( &box_two,(1<<24),100,100,28,28,tpage,0,23,tim->cx,tim->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
