...
// Author: George Bain
// Date: July 12, 1998
// Description: Chapter 7: Scrolling Example 3 - Paralax Scrolling 
...
// background
#define B_LEFT_ADDR  (0x80090000)
#define B_RIGHT_ADDR (0x80096000)

// foreground
#define F_LEFT_ADDR  (0x8009C000)
#define F_RIGHT_ADDR (0x800A2000)
...
GsSPRITE b_left_screen, b_right_screen;	// background
GsSPRITE f_left_screen, f_right_screen; // foreground
short fore_counter=0;					// duh!
short back_counter=0;					// duh!

//--------------------------------------------------------------------------
// Function: main()
// Description: Scrolling Example 3 - Paralax Scrolling
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
    ...
    while( !DONE )
      {
		 ...
		 FntPrint(fnt_id[0], "~c900Scrolling Example 3 - Paralax Scrolling");
...
// Description: Change x position of sprite based on our back counter and
...
void DrawBackground( void )
 {
	...
	b_left_screen.x = back_counter;
	GsSortFastSprite( &b_left_screen, &world_ordering_table[output_buffer_index], 0 );
	
	b_right_screen.x = back_counter + (SCREEN_WIDTH/2); 
	GsSortFastSprite( &b_right_screen, &world_ordering_table[output_buffer_index], 0 );
	
	b_left_screen.x = back_counter + SCREEN_WIDTH;
	GsSortFastSprite( &b_left_screen, &world_ordering_table[output_buffer_index], 0 );
	
	b_right_screen.x = back_counter + SCREEN_WIDTH + (SCREEN_WIDTH/2);
	GsSortFastSprite( &b_right_screen, &world_ordering_table[output_buffer_index], 0 );
...
// Description: Change x position of sprite based on our fore counter and
//              register sprites in OT
// Parameters: none
// Returns: none
// Notes: N/A
...
void DrawForeground( void )
 {
	...
	f_left_screen.x = fore_counter;
	GsSortFastSprite( &f_left_screen, &world_ordering_table[output_buffer_index], 0 );
	
	f_right_screen.x = fore_counter + (SCREEN_WIDTH/2); 
	GsSortFastSprite( &f_right_screen, &world_ordering_table[output_buffer_index], 0 );
	
	f_left_screen.x = fore_counter + SCREEN_WIDTH;
	GsSortFastSprite( &f_left_screen, &world_ordering_table[output_buffer_index], 0 );
	
	f_right_screen.x = fore_counter + SCREEN_WIDTH + (SCREEN_WIDTH/2);
	GsSortFastSprite( &f_right_screen, &world_ordering_table[output_buffer_index], 0 );
...
//--------------------------------------------------------------------------
// Function: Scroll()
// Description: Decrement scroll counters at different speed to give the
//              effect of parallax scrolling.  The foreground image moves
//              faster then the backgroud image.  We then reset our counters
//              when we reach (0-SCREEN_WIDTH) to wrap around our images
// Parameters: none
// Returns: none
// Notes: N/A
//--------------------------------------------------------------------------
...
void Scroll( void )
 {
	...
	if( fore_counter <= -SCREEN_WIDTH )
	    fore_counter = 0;

	if( back_counter <= -SCREEN_WIDTH )
	    back_counter = 0;
...
void InitBackground( void )
 {	 	
	...
	// left side
    left = ReadTIM((u_long*)B_LEFT_ADDR);    
	tpage = GetTPage(1,0,left->px,left->py);		
	InitSprite( &b_left_screen,(1<<24),0,0,160,150,tpage,0,0,left->cx,left->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
	
	// right side
    right = ReadTIM((u_long*)B_RIGHT_ADDR);	
	tpage = GetTPage(1,0,right->px,right->py);		
	InitSprite( &b_right_screen,(1<<24),160,0,160,150,tpage,0,0,right->cx,right->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
...
void InitForeground( void )
 {	 	
	...
	// left side
    left = ReadTIM((u_long*)F_LEFT_ADDR);    
	tpage = GetTPage(1,0,left->px,left->py);		
	InitSprite( &f_left_screen,(1<<24),0,100,160,150,tpage,0,0,left->cx,left->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
	
	// right side
    right = ReadTIM((u_long*)F_RIGHT_ADDR);	
	tpage = GetTPage(1,0,right->px,right->py);		
	InitSprite( &f_right_screen,(1<<24),160,100,160,150,tpage,0,0,right->cx,right->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
