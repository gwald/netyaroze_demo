...
// Author: George Bain
// Date: July 10, 1998
// Description: Chapter 2: Graphic Example 4 - Open, Display 4-bit TIM
...
//--------------------------------------------------------------------------
// Function: main()
// Description: Graphic Example 4 - Open, Display 4-bit TIM
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
    ...
    while( !DONE )
      {
		...
		FntPrint(fnt_id[0], "~c900 Graphic Example 4 - Open, Display 4-bit TIM");		
...
void InitSprite( void )
 {
	...
	sprite.attribute = 0; // 4bit
	
	sprite.x = 0;
	sprite.y = 50;

	sprite.w = tim.pw*4;  // (width * 4) = rendered width of 4-BIT tim
	sprite.h = tim.ph;
...
void ReadTIM( u_long *addr )
 { 		
    ...
    // ff CLUT exists, transfer it to VRAM 
 	if( (tim.pmode >> 3) & 0x01 ) 
 	  {
 	  	rect.x = tim.cx;
	  	rect.y = tim.cy;
