...
// Author: George Bain
// Date: July 10, 1998
// Description: Chapter 2: Graphics Example 8 - Semi-Transparent Sprites
...
//--------------------------------------------------------------------------
// Function: main()
// Description: Graphics Example 8 - Semi-Transparent Sprites
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
    ...
    while( !DONE )
      {
		 ...
		 // font print	     
		 FntPrint(fnt_id[0], "~c900 Graphics Example 8 - Semi-Transparent Sprites ");

		 if( (sprite.attribute) & (1<<30) )
		     FntPrint(fnt_id[1], "~c900 Semi-Trans ON ");
		 else
		     FntPrint(fnt_id[1], "~c900 Semi-Trans OFF "); 
		 ...
		 if( PAD_PRESS(buffer1,PAD_SQUARE) )
		   {
		     sprite.attribute >>= 16;     // clear bits, turn semi-trans OFF
			 sprite.attribute |= (1<<24); // set 8-bit sprite
			 
		   }

		 if( PAD_PRESS(buffer1,PAD_CIRCLE) )
		   {  			 
			 sprite.attribute |= (1<<30); // semi-trans ON
										  // 0.5 x Back + 0.5 Forward
		   }

		 if( PAD_PRESS(buffer1,PAD_L1) )
		   { 			 
			 sprite.attribute |= (1<<28); // 1.0 x Back + 1.0 Forward
			 
		   }
		 if( PAD_PRESS(buffer1,PAD_R1) )
		   { 			 
			 sprite.attribute |= (1<<29); // 1.0 x Back - 1.0 Forward
			 
		   }
		 if( PAD_PRESS(buffer1,PAD_R2) )
		   { 
		     			 
			 sprite.attribute |= (1<<28); // 1.0 x Back + 0.25 Forward
			 sprite.attribute |= (1<<29);
...
void InitSprite( void )
 {
	...
	sprite.w = (tim.pw*2);  // (width*2)  = rendered 8bit TIM
	sprite.h = tim.ph;
