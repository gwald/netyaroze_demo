...
// Author: George Bain
// Date: July 16, 1998
// Description: Chapter 8: A.I. Example 1 - Attack and Evade 
...
GsSPRITE box_one, box_two;		    // our sprites

//--------------------------------------------------------------------------
// Function: main()
// Description: A.I. Example 1 - Attack and Evade
// Parameters: none
// Returns: int
// Notes: N/A
//--------------------------------------------------------------------------
    ...
    while( !DONE )
      {
		...
		FntPrint(fnt_id[0], "~c900A.I. Example 1");
		FntPrint(fnt_id[1], "~c900Attack and Evade");
		FntPrint(fnt_id[2], "~c990Hold 'O' button for Attack mode");
		...
		if( PAD_PRESS(buffer1,PAD_CIRCLE) )
		  {
		    Attack();
			FntPrint(fnt_id[3], "~c909Attack");
		  }
		else
		  {
		    Evade();
			FntPrint(fnt_id[3], "~c909Evade");

 			if( box_two.x > (SCREEN_WIDTH-box_two.w) )
			    box_two.x = (SCREEN_WIDTH-box_two.w);
			if( box_two.x < 0 )
			    box_two.x =0;

			if( box_two.y > (SCREEN_HEIGHT-box_two.h) )
			    box_two.y = (SCREEN_HEIGHT-box_two.h);
			if( box_two.y < 0 )
			    box_two.y = 0;
...
void InitBoxOneSprite( void )
 {	 	
	...
	InitSprite( &box_one,(1<<24),0,0,44,38,tpage,0,24,tim->cx,tim->cy,
...
void InitBoxTwoSprite( void )
 {	 	
	...
	tim = ReadTIM((u_long*)TIM_ADDR);    
	tpage = GetTPage(1,0,tim->px,tim->py);		
	InitSprite( &box_two,(1<<24),50,50,44,22,tpage,0,0,tim->cx,tim->cy,
	            0x80,0x80,0x80,0,0,ONE,ONE,0 );
...
void UpdateScreen( void )
 {
    ...
    GsSortSprite(&box_one,&world_ordering_table[output_buffer_index],0);        
    GsSortSprite(&box_two,&world_ordering_table[output_buffer_index],0);
