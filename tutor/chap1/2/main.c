...
// Author: George Bain
// Date: August 3, 1998
// Description: Chapter 1 - Mouse Example
 ...
 {	          
    ...
    while( done )
      {	 		 	     
         ...
         FntPrint(fnt_id[0]," Mouse Example ");	
         	  
	   	 if( MOUSE_PRESS(buffer1,MOUSE_LEFT) )
	       {
		     FntPrint(fnt_id[1]," LEFT ");
			 sprite.rotate -= 4096;
   	       }

         if( MOUSE_PRESS(buffer1,MOUSE_RIGHT) )
	       {
		     FntPrint(fnt_id[2]," RIGHT ");
			 sprite.rotate += 4096;
   	       }

   	     if( !MOUSE_PRESS(buffer1,MOUSE_NOKEY) )
	       {
		     FntPrint(fnt_id[2]," NO BUTTONS PRESSED ");
   	       }	  
		   			 
     
         sprite.x += MOUSE_X(buffer1);
	     sprite.y += MOUSE_Y(buffer1);

	   	
	     FntPrint(fnt_id[3]," \n x offset:%d, y offset:%d ",
	              MOUSE_X(buffer1), MOUSE_Y(buffer1) );
	       
	     if( MOUSE_PRESS(buffer1,MOUSE_RIGHT) && 
	         MOUSE_PRESS(buffer1,MOUSE_LEFT) ) 
	       {
...
// Description: Load TIM and setup our sprite structure
// Parameters: none
// Returns: none
// Notes: N/A
//--------------------------------------------------------------------------
